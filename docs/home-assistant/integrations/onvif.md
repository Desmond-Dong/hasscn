...
| Extra FFmpeg arguments | Extra options to pass to `ffmpeg`, e.g., image quality or video filter options. More details in [`ffmpeg` integration](/home-assistant/integrations/ffmpeg). |
...
如果您在使用该传感器时遇到问题，请参阅[故障排除部分](/home-assistant/integrations/ffmpeg/#troubleshooting)。
