...
:::note
您必须在 `start`、`end` 和 `duration` 中**恰好提供 2 个**。
<br/>
您可以使用 [template extensions](/home-assistant/docs/configuration/templating/#home-assistant-template-extensions)（例如 `now()` 或 `as_timestamp()`）来处理动态日期，如下方示例所示。
:::
