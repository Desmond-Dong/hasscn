...
启用 [Ring 集成](/home-assistant/integrations/ring)后，您就可以使用按钮平台。目前支持通过对讲开门。

### 摄像头

启用 [Ring 集成](/home-assistant/integrations/ring)后，您就可以使用摄像头平台。
...
启用 [Ring 集成](/home-assistant/integrations/ring)后，您就可以使用传感器平台。目前支持电池电量和 Wi-Fi 信号。
...
启用 [Ring 集成](/home-assistant/integrations/ring)后，您就可以使用开关平台。
...
启用 [Ring 集成](/home-assistant/integrations/ring)后，您就可以使用灯光平台。对于每个支持灯光的摄像头（例如泛光灯摄像头），都会添加一个灯光实体。

### 数值实体

启用 [Ring 集成](/home-assistant/integrations/ring)后，您就可以使用 number 平台。
...
您可以结合使用 [downloader](/home-assistant/integrations/downloader) 与 [automation](/home-assistant/integrations/automation) 或 [python_script](/home-assistant/integrations/python_script)，将 Ring Door Bell 捕获的最新视频保存到本地。
首先，在配置中启用 [downloader](/home-assistant/integrations/downloader) 集成，将以下内容添加到 `configuration.yaml`。
