...
:::
:::tip
如果你启用了 RTSP 服务器，就可以通过其他 Home Assistant 摄像头平台连接到摄像头。不过，这个 RTSP 服务器会禁用非常实用的米家应用。为了同时保留 Home Assistant 兼容性 _以及_ 原生应用的可用性，此平台通过 FTP 获取视频。
...
所有被 [`ffmpeg` 摄像头](/home-assistant/integrations/camera.ffmpeg/)支持的选项，都可以通过 `ffmpeg_arguments` 配置参数使用。
...
有关如何配置要跟踪的人员，请参阅[设备追踪器集成页面](/home-assistant/integrations/device_tracker/)。
