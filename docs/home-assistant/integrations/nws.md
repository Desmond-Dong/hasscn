...
为配置中的每个条目创建一个天气实体。通过 [`weather.get_forecasts` 操作](/home-assistant/integrations/weather#action-weatherget_forecasts) 提供每小时和日/夜预报。为每个预测提供的时间是预测的开始时间。传感器在配置后也会被创建为禁用实体，并且可由用户启用。

## 操作 `nws.get_forecasts_extra`

`nws.get_forecasts_extra` 以类似于 `weather.get_forecasts` 的形式提供额外数据。请参阅 [`weather.get_forecasts` 文档](/home-assistant/integrations/weather#action-weatherget_forecasts)。
...
“forecast”是给定时间点的预测条件列表，这些条件不是从“weather.get_forecasts”返回的。  仍为上下文提供“datetime”和“is_daytime”属性。
