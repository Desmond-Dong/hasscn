...
To enable the TCP integration, add the following lines to your "`configuration.yaml`" file.
:::tip
...
timeout:
  ...
  type: integer
value_template:
  description: Defines a [template](/home-assistant/docs/configuration/templating/#processing-incoming-data) to extract the value. By default it's assumed that the entire response is the value.
  required: false
  type: template
unit_of_measurement:
...
value_on:
  ...
  type: string
value_template:
  description: Defines a [template](/home-assistant/docs/configuration/templating/#processing-incoming-data) to extract the value.
  required: false
  type: template
