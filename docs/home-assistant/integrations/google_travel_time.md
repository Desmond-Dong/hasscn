...
`google_travel_time.get_travel_times` 操作用于获取两个位置之间的备选路线和行程时间。它会填充[响应数据](/home-assistant/docs/scripts/perform-actions#use-templates-to-handle-response-data)。
...
`google_travel_time.get_transit_times` 操作用于获取两个位置之间使用公共交通的备选路线和行程时间。它会填充[响应数据](/home-assistant/docs/scripts/perform-actions#use-templates-to-handle-response-data)。
