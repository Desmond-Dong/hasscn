...
请注意，`verify_ssl` 默认为 `True`，并且任何远程主机都需要在您的 [`allowlist_external_urls`](/home-assistant/integrations/homeassistant/#allowlist_external_urls) 列表中。Discord 将附件大小限制为 8MB，因此任何超过此大小的内容将被跳过并在错误日志中注明。
...
要有效使用通知，请参阅[自动化入门页面](/home-assistant/getting-started/automation/)。
