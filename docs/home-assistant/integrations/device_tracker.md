...
多个 device tracker 可以并行使用，例如 [Owntracks](/home-assistant/integrations/owntracks/) 和 [Nmap](/home-assistant/integrations/nmap_tracker/)。设备状态将由最后一次上报的来源决定。
...
- **Home**：您的跟踪设备位于 [home zone](/home-assistant/integrations/zone#home-zone) 中，由网络或基于蓝牙的在家状态检测发现。如果您使用的是包含坐标的在家状态检测方法，那么当设备位于某个区域内时，状态会等于该区域名称（区分大小写）。
