---
title: "MQTT Switch"
description: 'The MQTT Switch integration lets you control your MQTT enabled switches. 本页属于 Home Assistant 中文文档，适合查阅安装部署、集成、自动化与日常使用说明。'
...
在理想情况下，MQTT 设备将有一个“state_topic”来发布状态更改。如果这些消息使用“RETAIN”标志发布，则 MQTT 交换机将在订阅后收到即时状态更新，并以正确的状态开始。否则，开关的初始状态将为“未知”。 MQTT 设备可以使用“None”有效负载将当前状态重置为“unknown”。

当“state_topic”不可用时，交换机将以乐观模式工作。在此模式下，开关将在每个命令后立即更改状态。否则，交换机将等待来自设备的状态确认（来自“state_topic”的消息）。在乐观模式下，初始状态设置为“False”/“off”。

即使“state_topic”可用，也可以强制使用乐观模式。如果遇到不正确的开关操作，请尝试启用它。

To use an MQTT switch in your installation, [add a MQTT device as a subentry](/home-assistant/integrations/mqtt/#configuration), or add the following to your `configuration.yaml` file.
...
为了进行检查，您可以使用“mosquitto”附带的命令行工具“mosquitto_pub”来发送 MQTT 消息。这允许您手动操作开关。首先，我们可以模拟为交换机发送的可用性消息：
...
您可以使用“mosquitto_pub”或其他 MQTT 工具手动将引脚 13 设置为高电平：
