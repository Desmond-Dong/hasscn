...
这些凭据与 [Nest](/home-assistant/integrations/nest)、[Google Mail](/home-assistant/integrations/google_mail) 和大多数其他 Google 集成的凭据相同。
...
The integration setup then gives you instructions to enter the [Application Credentials](/home-assistant/integrations/application_credentials/) and authorize Home Assistant to connect to Google Tasks.
...
该集成将为每个任务列表创建一个[待办事项实体](/home-assistant/integrations/todo)。
...
Google Tasks 集成提供待办事项实体支持的所有动作，包括检索、创建、更新和删除待办事项项目的动作以及上述字段。有关更多详细信息，请参阅 [待办事项：动作](/home-assistant/integrations/todo#actions) 文档。
...
尝试配置集成时，Google OAuth 流程显示消息 *The OAuth client was not found* 和 *Error 401: invalid_client*。
...
1. 按照[应用程序凭据](/home-assistant/integrations/application_credentials/#deleting-application-credentials)中的说明删除任何现有凭据。
1. 按照上面[前提条件](#prerequisites)中的步骤操作。
1. 按照上面[配置](#configuration)中的步骤操作。
...
系统会询问您是否要移除[应用程序凭据](/home-assistant/integrations/application_credentials/)，如果您不再计划在 Home Assistant 中使用 Google Tasks，可以这样做。您可能还想移除在前提条件中创建的 Google Cloud Console 中的任何凭据，如果它们不再与您的任何其他 Home Assistant 集成一起使用。