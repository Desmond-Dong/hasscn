...
默认情况下，GEM 每 5 秒发送一次更新。这会产生大量数据，而 [`recorder`](/home-assistant/integrations/recorder) 集成用于历史记录的数据库并不擅长处理如此多的数据，因此建议您配置 [`influxdb`](/home-assistant/integrations/influxdb) 集成，并将 GEM 传感器从 `recorder` 中排除。
