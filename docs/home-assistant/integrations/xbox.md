...
You may then add the credentials to [Application Credentials](/home-assistant/integrations/application_credentials/) and then setup the integration.
...
在报告问题时，请启用 [debug logging](/home-assistant/docs/configuration/troubleshooting/#debug-logs-and-diagnostics)。重启集成，并在问题再次出现后立即关闭调试日志。调试日志文件会自动下载。
