...
To use an MQTT water heater in your installation, [add an MQTT device as a subentry](/home-assistant/integrations/mqtt/#configuration), or add the following to your `configuration.yaml` file.
...
对于所有“*_state_topic”，可以指定一个模板，用于呈现这些主题上的传入有效负载。此外，可以将适用于所有状态主题的默认模板指定为“value_template”。如果您收到的有效负载是 JSON 格式等，这会很有用。由于在 JSON 中，带引号的字符串（例如“foo”）只是一个字符串，因此也可以用于取消引号。

假设您通过“mode_state_topic”接收到操作模式“off”，但该模式实际上被称为“off”，您可以执行以下操作：
...
类似地，对于“*_command_topic”，可以指定一个模板来呈现这些主题的传出负载。
