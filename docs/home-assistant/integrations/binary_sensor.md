...
当您添加设备集成时，某些二值传感器会自动创建。例如，添加 [ecobee 集成](/home-assistant/integrations/ecobee/) 将创建一个二值传感器来检测房间占用情况。其他二值传感器可以使用[模板集成](/home-assistant/integrations/template/)或使用[输入布尔助手](/home-assistant/integrations/input_boolean)手动创建。
...
- **battery_charging**：`on` 表示正在充电，`off` 表示未充电
- **carbon_monoxide**：`on` 表示检测到一氧化碳，`off` 表示无一氧化碳（正常）
- **cold**：`on` 表示冷，`off` 表示正常
- **connectivity**：`on` 表示已连接，`off` 表示已断开
- **door**：`on` 表示开启，`off` 表示关闭
- **garage_door**：`on` 表示开启，`off` 表示关闭
