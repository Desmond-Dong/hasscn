...
调用 `media_player.play_media` 动作时，`media_content_type` 必须设为 "favorite_item_id"，`media_content_id` 必须设为收藏项的键值部分。
...
在 `media_player.play_media` 动作中，Sonos 支持多种 `media_content_id` 格式，最常见的是 URI。例如，Spotify 和 Tidal 的分享链接都可直接使用。你也可以播放[托管在 Plex 服务器上的音乐](/home-assistant/integrations/plex#sonos-playback)。如果 Sonos 设备可直接访问 URI，也可使用本地或远程媒体文件的 HTTP/HTTPS 直链，但具体编码格式支持情况可能不同。
...
你在家中运行了 [Plex Media Server](/home-assistant/integrations/plex#sonos-playback) 吗？Sonos 集成也能与其配合。以下示例会直接从你的 Plex 服务器播放音乐：
...
除[标准媒体播放器动作](/home-assistant/integrations/media_player/#actions)外，Sonos 集成还提供多种自定义动作。
...
:::note
播放队列不会被快照，恢复前必须保持不变。使用 `media_player.play_media` 是安全的，可用于播放通知音，包括 [TTS](/home-assistant/integrations/tts/) 播报。
...
如果你的 Home Assistant 实例有多个 IP 地址，你可以通过 [Network](/home-assistant/integrations/network/) 集成选择 Sonos 自动发现所使用的特定 IP 地址。通常仅在 Sonos 音箱位于默认网卡无法到达的网段时才需要这样做。
