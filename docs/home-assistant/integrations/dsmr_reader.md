...
此集成依赖于现有的 DSMR Reader 应用程序设置。它处理由 MQTT 发布功能触发的事件，以在 Home Assistant 中创建传感器实体。此集成使用发布到 MQTT 代理的数据，无论应用程序如何或在哪里安装。相比之下，[DSMR](/home-assistant/integrations/dsmr/) 集成直接在 Home Assistant 中连接到智能电表，不使用 DSMR Reader 应用程序。