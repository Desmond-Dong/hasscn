...
启用且正常运行 [Bluetooth](/home-assistant/integrations/bluetooth) 集成后，Medcom Bluetooth 集成会自动发现设备。它会将检测到的每个 Inspector 设备以其蓝牙 MAC 地址作为序列号列出。
...
此集成会为每个检测到的 Inspector BLE 设备添加一个每分钟计数（"CPM"）传感器。如果您希望将该 CPM 读数转换为其他单位，可参阅 Inspector BLE 手册；这可以通过自定义 [template sensor](/home-assistant/integrations/template) 实现。
