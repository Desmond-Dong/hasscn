...
:::tip
如果您想在不使用 YAML 的情况下实现一个开关，可以考虑使用 [template switch helper](/home-assistant/integrations/template/#switch)。将 Wake on LAN 按钮作为开机操作，使用 [ping](/home-assistant/integrations/ping) 传感器表示状态，再用第三个服务作为关机操作。
...
`wake_on_lan.send_magic_packet` 操作用于向支持 'Wake on LAN' 的设备发送 _magic packet_ 以唤醒设备。
