...
:::tip
如果你想查看如何在 Home Assistant 中使用 webhook 设置 Traccar Client，请参阅 [Traccar Client](/home-assistant/integrations/traccar/) 集成文档。
...
Traccar Server 集成的选项允许你定义想要监听的事件列表。这些事件会作为 [events](/home-assistant/docs/configuration/events/) 发送到 Home Assistant。

所有事件都会以 `traccar_` 作为前缀，后面跟上事件名称的 snake_case 形式。
例如：
...
这些设备在 Home Assistant 中会关联相应的[实体](#entities)，你可以在[自动化](/home-assistant/docs/automation)、[脚本](/home-assistant/docs/scripts)中使用它们，也可以将其显示在[仪表板](/home-assistant/dashboards)上。
...
Traccar Server 集成会为 Traccar Server 中注册的每台设备创建一个 [binary_sensor](/home-assistant/integrations/binary_sensor) 实体，用于显示 Traccar Server 上报的运动状态。
...
Traccar Server 集成会为 Traccar Server 中注册的每台设备创建一个 [binary_sensor](/home-assistant/integrations/binary_sensor) 实体，用于显示 Traccar Server 上报的在线状态。
...
Traccar Server 集成会为 Traccar Server 中注册的每台设备创建一个 [device tracker](/home-assistant/integrations/device_tracker) 实体。
...
Traccar Server 集成会为 Traccar Server 中注册的每台设备创建一个 [sensor](/home-assistant/integrations/sensor) 实体，用于显示 Traccar Server 上报的地址。
...
Traccar Server 集成会为 Traccar Server 中注册的每台设备创建一个 [sensor](/home-assistant/integrations/sensor) 实体，用于显示 Traccar Server 上报的海拔。
...
Traccar Server 集成会为 Traccar Server 中注册的每台设备创建一个 [sensor](/home-assistant/integrations/sensor) 实体，用于显示 Traccar Server 上报的剩余电池百分比。
...
Traccar Server 集成会为 Traccar Server 中注册的每台设备创建一个 [sensor](/home-assistant/integrations/sensor) 实体，用于显示 Traccar Server 上报的地理围栏。
...
Traccar Server 集成会为 Traccar Server 中注册的每台设备创建一个 [sensor](/home-assistant/integrations/sensor) 实体，用于显示 Traccar Server 上报的速度。
