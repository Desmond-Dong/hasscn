---
title: "MQTT Siren"
description: 'The MQTT Siren integration lets you control your MQTT enabled sirens and text based notification devices. 本页属于 Home Assistant 中文文档，适合查阅安装部署、集成、自动化与日常使用说明。'
...
在理想情况下，MQTT 设备将有一个“state_topic”来发布状态更改。如果这些消息使用“RETAIN”标志发布，MQTT 警报器将在订阅后收到即时状态更新，并以正确的状态开始。否则，警报器的初始状态将为“假”/“关”。

当“state_topic”不可用时，警报器将以乐观模式工作。在此模式下，警报器将在每次命令后立即更改状态。否则，警报器将等待设备的状态确认（来自“state_topic”的消息）。

即使“state_topic”可用，也可以强制使用乐观模式。如果操作不正确，请尝试启用它。

To use an MQTT siren in your installation, [add a MQTT device as a subentry](/home-assistant/integrations/mqtt/#configuration), or add the following to your `configuration.yaml` file.
...
为了进行检查，您可以使用“mosquitto”附带的命令行工具“mosquitto_pub”来发送 MQTT 消息。这允许您手动操作警报器：
