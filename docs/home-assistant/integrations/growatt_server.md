...
- **device_id** *(string, optional)*：逆变器的设备 ID。仅在存在多个设备时需要
- **segment_id** *(integer, required)*：时间段编号（1-9）
- **batt_mode** *(string, required)*：系统的能量优先模式：
  ...
  :::
- **start_time** *(time, required)*：该时间段的开始时间（HH:MM 格式）
- **end_time** *(time, required)*：该时间段的结束时间（HH:MM 格式）
...
- **device_id** *(string, optional)*：MIN 逆变器的设备 ID。仅在存在多个设备时需要
...
有关调试日志的更多信息，请参阅 [debug logs and diagnostics](/home-assistant/docs/configuration/troubleshooting/#debug-logs-and-diagnostics)。

### 报告问题

如果您遇到了无法通过上述故障排除步骤解决的问题：

1. 为该集成启用 [debug logging](/home-assistant/docs/configuration/troubleshooting/#debug-logs-and-diagnostics)。
...
5. 如果可以，也请下载该集成的 [diagnostics](/home-assistant/integrations/diagnostics) 数据。
