---
title: LiteJet
description: 关于如何在 Home Assistant 中设置 LiteJet 集线器、Centralite Elegance 或 Centralite Jetstream 的说明。
ha_category:
  - Light
  - Scene
  - Switch
...
- **number**（*必填*）：要监控的开关编号。
- **held_more_than**（*可选*）：触发器可激活前，开关必须至少被按住的时间。
- **held_less_than**（*可选*）：触发器可激活前，开关允许被按住的最长时间。
