...
description: '<a href=''/home-assistant/integrations/版本/0.61''<img src=''/home-assistant/images/blog/2018-01-0.61/components.png'' style=''border: 0;box-shadow: none;''</a。'
---
...
[@amelchio] 让 Home Assistant 启动速度 _更快_ 了。现在所有服务描述都会在前端需要时才加载，而不是在启动阶段全部加载。为此我们加强了服务命名规范检查，并修复了少数不符合规范的平台：
...
- Threshold 二元传感器现已支持区间范围，也就是说你现在可以同时设置 `upper` 和 `lower`。([@DanNixon] - [#11110]) ([binary_sensor.threshold docs]) (breaking change)
- Steam game 平台有以下变更：
  ...
  - `Play` 和 `Trade` 状态已分别重命名为 `looking_to_play` 和 `looking_to_trade`。
    ([@frwickst] - [#11182]) ([传感器.steam_online docs]) (breaking change)
- [`tile`][device_tracker.tile docs] 平台现在默认只显示处于活动状态的 Tile；如果想显示全部 Tile（包括过期或未激活的），需要将 `show_inactive` 设为 `true`。同时移除了以下状态属性：`last_seen` 和 `last_updated`。([@bachya] - [#11172]) ([device_tracker.tile docs]) (breaking change)
...
- [`egardia` 报警面板][alarm_control_panel.egardia docs] 平台不再需要用户额外运行一个单独的 Egardiaserver 组件，现在它也可以直接运行在 HASS.io 上。([@jeroenterheerdt] - [#11344]) ([alarm_control_panel.egardia docs]) (breaking change)
...
- Insteon local 设备 will now use their address as the `entity_id` and `name`. The friendly name can be customized using the [standard customization configuration](/home-assistant/docs/configuration/customizing-devices/). ([@camrun91] - [#11088]) ([insteon_local docs]) ([风扇.insteon_local docs]) ([灯光.insteon_local docs]) ([开关.insteon_local docs]) (breaking change)
