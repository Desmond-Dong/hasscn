...
At the moment Home Assistant only supports one [MQTT broker](/home-assistant/docs/MQTT/). This means that you can't subscribe to topics which are located on different brokers.

<!--more-->

## Subscribe to the TTN Broker

To check what your 设备 are sending, subscribe to the topic `+/devices/+/up` with a command-line tool like `mosquitto_sub`. The `<Region>` is the postfix of the **Handler** entry in your **Application 概述**. `<AppID>` is the **Application ID** and `<AppKey>` is your access key. 
...
The payload contains details about the 设备 itself and the 传感器 data. The 传感器 data is stored in `payload_fields`. Depending on the 设备 配置 it may contain a single value or multiple values.

## The relay

To be able to work locally with the MQTT data that is received from the 设备 connected to TTN, we need to transfer it to the local broker. With this simple 脚本 below all messages from a given 设备 are re-published on your local MQTT broker after they are received. Modify the 脚本 with your details as outlined in the previous section.
...
Save it and run it. As soon as a MQTT message is received from your 设备 you should see it on your local broker (here 192.168.0.2) if you subscribe to `#` or the topic given in the 脚本 above `home/ttn/garden_temp`.
...
All we would need now, is a [`mqtt` sensor](/home-assistant/integrations/sensor.MQTT/) with a `value_template`. With a sophisticated custom 传感器 it would be possible to displaying a little more than just the 状态. The 设备 is only sending the temperature `{"temperature": 7.5}` but there are other details available which the 传感器 should show.
...
Now create the [`mqtt_ttn` sensor](/home-assistant/integrations/sensor.MQTT/) entry for your 设备.
...
This solution is not production-ready, scalable or stable but it could fill the gape till Home Assistant is able to connect to multiple MQTT brokers. If you have multiple 设备 relay all messages to your local broker and add a 配置 variable to `mqtt_ttn` 传感器 which allows you to select the 设备.
