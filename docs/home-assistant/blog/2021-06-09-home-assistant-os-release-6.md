...
<img title='Home Assistant OS 发布 6' alt='Home Assistant OS 发布 6 Logo'
     src='/home-assistant/images/blog/2021-06-11-os6/social.png' style='border: 0;box-shadow: none;'>
</a>
...
- OS Agent for better OS 集成 with Supervisor
- Support for additional Wi-Fi and 蓝牙 设备
...
- [devices Support](#devices-support)
  ...
  - [Additional Wi-Fi and 蓝牙 devices support](#additional-WiFi-and-蓝牙-devices-support)
...
Operating System 发布 6 comes with a new, Go written daemon called OS Agent.
...
Supervisor and 前端 is still being developed.

### Under the Hood

Under the hood, the OS was updated to the latest upstream Linux 5.10 kernel
as well as Buildroot 2021.02.1. The latest Buildroot 发布 brings tons of
new software versions along with bug and security fixes. Some key components
which received an 更新 were systemd 247 and the Docker 容器 Engine
20.10.6.
...
- The Home Assistant CLI is now started on tty1 instead of the login prompt. The
  HA CLI banner shows the IP and other 设备 information without the need
...
All 树莓派 versions were updated to use the Linux Kernel 5.10 from the
树莓派 team, just like 树莓派 OS. The latest 蓝牙 firmware for
CYW43455 (树莓派 3 B+/4) with security fixes is now part of the Raspberry
Pi images.

### Generic x86-64

In Home Assistant OS 6 the board/image "Intel NUC" was renamed to "Generic
x86-64". Existing "Intel NUC" 安装 will automatically migrate to the
"Generic x86-64" image on 更新.
...
设备 is on a best effort basis.

Other Changes:

- The Intel IGC network driver supports Intel network 卡片 such as I225-LM/I225-V
  used on some newer Intel NUC 设备.

- With Home Assistant OS 6 the latest 版本 of the Barebox bootloader 2021.05.0
  which fixes boot from hard disks larger than 2TB.

- The ACPI kernel driver for Thinkpad 设备 is enabled.
...
### Additional Wi-Fi and 蓝牙 设备 Support

Ralink rt27xx/rt28xx/rt30xx, as well as Realtek 87xx/88xx 设备, are now
supported on boards that don't have onboard Wi-Fi/蓝牙. This is also
...
part of the image so that Intel Wi-Fi/蓝牙 设备 can be used in
...
for RPI-RF-MOD or HM-MOD-RPI-PCB 设备. This together with the
[RaspberryMatic CCU Add-On](https://github.com/jens-maus/RaspberryMatic/wiki/安装-HomeAssistant)
offers the full functionality of RaspberryMatic embedded in Home Assistant.

### Google Coral PCIe AI Accelerator Support

[@ryddler] enabled the driver for Google Coral PCIe TPU 设备. This allows to
use Google Coral Mini PCIe Accelerator or M.2 Accelerator on all boards
supporting PCIe.

That's all Folks! Now go out and 更新!
