---
title: 社区精选：第一期！
description: 与众不同的界面设计、智能厕纸架，以及 DIY 智能信箱
---

早在 [2015 年][community-highlights-2015] 和 [2016 年][community-highlights-2016]，我们就尝试过推出一个名为 “Community Highlight” 的系列博客，介绍社区里那些非常精彩的 Home Assistant 玩法。

最近，这个想法又以每次发布说明中的 “In Other News” 形式回归了。不过 Home Assistant 的开发节奏太快了！结果就是那个栏目常常埋在很靠后的发布说明里，有点可惜，因为社区里真的有太多值得一看的作品。

所以今天，我们正式带来：社区精选，第一期！

严格来说它并不是第一次，但我们这次算是重启这个系列，对吧？

希望你会喜欢，欢迎在下方评论区告诉我们你的想法！  
也欢迎来我们的 [Discord #lounge 频道][chat] 聊天。

../Frenck

## 不一样的前端设计思路

Mattias Persson 展示了一个很有意思的 Lovelace UI 设计方向。
视觉效果非常惊艳；我们都不知道该从哪里讲起，你先看下面的视频感受一下：

<div class='videoWrapper'>
<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/727tFhqpFAE" frameborder="0" allowfullscreen></iframe>
</div>

Mattias 在社区论坛里[发了帖子][different-take]，详细介绍了实现细节、更多截图、所用组件以及 GitHub 仓库链接。如果你也想自己复刻，这些信息基本都齐了。感谢 Mattias！

## 智能厕纸架

说真的，我们都懂那种尴尬：上厕所时才发现没纸了。

在我们的 [subreddit][reddit] 上，for-the-berik 因为这个问题决定亲手改造。他把厕纸架改成了智能版本，使用 ESP8266、称重传感器和 [ESPHome][esphome]，让 Home Assistant 可以知道架子上还剩几卷厕纸。

点击下面的视频看看它是怎么工作的。

<blockquote class="reddit-card"><a href="https://www.reddit.com/r/homeassistant/comments/elwvro/smart_toiletroll_holder_with_esphome_and/">Smart toiletroll holder with Esphome and Homeassistant</a> from <a href="http://www.reddit.com/r/homeassistant">r/homeassistant</a></blockquote>
<script async src="//embed.redditmedia.com/widgets/platform.js" charset="UTF-8"></script>

## 简单好做的智能信箱传感器

同样来自我们的 [subreddit][reddit]：choketube 分享了他很自豪的信箱传感器方案。
他在信箱里放了一个 Hue 运动传感器，信箱一开就会触发，简单又聪明！

他在下面的视频里演示了这个信箱方案。

<blockquote class="reddit-card"><a href="https://www.reddit.com/r/homeassistant/comments/ejgzeb/proud_of_my_new_automation_hue_sensor_in_my/">Proud of my new automation - Hue sensor in my Mailbox</a> from <a href="http://www.reddit.com/r/homeassistant">r/homeassistant</a></blockquote>

## 想给下一期投稿？

你是否见过（或亲手做过）一些很棒、很有趣、很独特、很惊艳、很有启发、很奇特或很好玩的 Home Assistant 作品？

[点击这里提交你的 Community Highlight 推荐](/home-assistant/suggest-community-highlight)。

另外，别忘了通过社交媒体分享你的创作：

- 发推并记得提到 [@home_assistant][twitter]
- 分享到我们的 [Facebook 群组][facebook-group]
- 发布到我们的 [subreddit][reddit]
- 在 Instagram @ [@homeassistant][instagram]
- 或来聊天，在 [Discord #lounge][chat] 给我们留言

下期再见！

[chat]: https://www.home-assistant.io/join-chat
[community-highlights-2015]: https://www.home-assistant.io/blog/2015/12/05/community-highlights/
[community-highlights-2016]: https://www.home-assistant.io/blog/2016/02/20/community-highlights/
[different-take]: https://community.home-assistant.io/t/a-different-take-on-designing-a-Lovelace-ui/162594
[esphome]: https://www.esphome.io
[facebook-group]: https://www.facebook.com/groups/HomeAssistant/
[instagram]: https://www.instagram.com/homeassistant/
[reddit]: https://www.reddit.com/r/homeassistant
[twitter]: https://www.twitter.com/home_assistant
