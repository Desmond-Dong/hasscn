---
title: Home Assistant Core 2025.5 完整更新日志
description: '以下是 Home Assistant Core 2025.5 版本包含的全部变更。 本页属于 Home Assistant 中文文档，适合查阅安装部署、集成、自动化与日常使用说明。'
---
...
- 替换 "controller_id" with friendly name in `homeworks` 错误 message ([@NoRi2909] - [#141550])
...
- 改进 performance of websocket_api _state_diff_event ([@bdraco] - [#141696])
...
- 迁移 emulated_roku to 使用 runtime_data to 修复 flakey 测试 ([@bdraco] - [#141795])
...
- 改进 performance of as_compressed_state ([@bdraco] - [#141800])
...
- 添加 类型化 hint on inherrited 属性 _message_callback for MQTT mixin classes ([@jbouwh] - [#142011])
...
- 使用 more common state for "foot_warmer_temp" in `sleepiq` ([@NoRi2909] - [#142048])
- 使用 common state for "wi_fi_strength" in `aquacell` ([@NoRi2909] - [#142047])
- 使用 common state for "ptc_level" in `xiaomi_miio` ([@NoRi2909] - [#142044])
...
- 修复 lying comment in ConfigEntriesFlowManager.async_finish_flow ([@emontnemery] - [#142146])
...
- 添加 missing test_all requirements ([@twasilczyk] - [#142036])
...
- 移除 FlowManager.async_post_init ([@emontnemery] - [#142462])
...
- 添加 full 测试 coverage for Fritz config_flow ([@chemelli74] - [#142418])
...
- 移除 redundant logging from UptimeRobot config_flow ([@chemelli74] - [#142940])
...
- 修复 homeaticip_cloud RuntimeWarnings ([@cdce8p] - [#142961])
...
- 更新 sky_hub to 移除 代码所有者 ([@RogerSelwyn] - [#143047])
...
- Explictly 设置 PARALLEL_UPDATES in ESPHome assist_satellite entity 平台 ([@bdraco] - [#143068])
...
- 添加 Homee wind_monitoring_state to numbers ([@Taraman17] - [#139848])
...
- 使用 common strings for "cannot_connect" in `imgw_pib` ([@NoRi2909] - [#143574])
...
- 使用 common strings for "already_in_progress" etc. in `music_assistant` ([@NoRi2909] - [#143570])
...
- 添加 more huawei_lte 传感器 描述 ([@scop] - [#143707])
...
- 使用 snapshot_platform in renault 测试 ([@epenet] - [#143864])
...
- 添加 翻译 for "energy_distance" and "wind_direction" in `random` ([@NoRi2909] - [#143994])
