---
title: 待办事项列表卡片
description: '待办事项列表卡片允许您向待办事项列表中添加、编辑、勾选和清除项目。 本页属于 Home Assistant 中文文档，适合查阅安装部署、集成、自动化与日常使用说明。'
---
# 待办事项列表卡片
...
2. 在 **实体** 下拉菜单中，选择您的列表类型。
   ...
   - 它来自默认安装的[购物清单集成](/home-assistant/integrations/shopping_list/)。
   - 这与 **待办事项列表** 仪表盘（可通过侧边栏访问）中的 **购物清单** 是同一个列表。
   ![待办事项卡片，列表实体](/home-assistant/images/dashboards/cards-todo.png).
3. 待办事项列表卡片可以显示来自不同[to-do list](/home-assistant/integrations/#to-do-list) 集成的列表，例如 **Bring!** 或 **Todoist**。
