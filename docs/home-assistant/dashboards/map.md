...
3. 默认情况下，您会在地图上看到房子 `[mdi:house]` 图标。它代表您的[家庭区域](/home-assistant/integrations/zone/#about-the-home-zone)。
   - 要更改您家庭的位置，您需要[在常规设置中编辑您家庭的位置](/home-assistant/docs/configuration/basic/#editing-the-general-settings)。

    ![编辑地图卡片设置](/home-assistant/images/dashboards/map_card_config.png)
4. 要了解如何在地图上显示其他区域，请按照[添加新区域](/home-assistant/integrations/zone/#adding-a-new-zone-or-editing-zones)的步骤操作。
5. 要在地图上显示其他元素，可以在**实体**下添加它们，或使用**地理位置来源**。
   ...
   - 有关存在检测的更多信息，请参阅[存在检测入门教程](/home-assistant/getting-started/presence-detection/)。
...
geo_location_sources:
  required: false
  description: 地理位置来源或 `source` 对象的列表（参见[下文](#geolocation-sources-选项)）。所有具有该来源的当前实体都将显示在地图上。有关有效来源，请参阅[地理位置](/home-assistant/integrations/geo_location/)平台。设置为 `all` 以使用所有可用来源。此选项、`show_all` 或 `entities` 配置选项是必需的。
  type: list
show_all:
  required: false
...
您可以指定一个或多个 `conditions`，在这种情况下，每个选定的实体都将针对每个条件进行测试，如果通过所有条件则显示。参见[可用条件](/home-assistant/dashboards/conditional/#condition-options)。对于接受 `entity` id 的条件，这将自动设置为正在测试的实体。
...
label_mode:
  required: false
  default: name
  description: 当设置为 `icon` 时，在标记中渲染实体的图标而不是文本。当设置为 `state` 或 `attribute` 时，将实体的状态或属性渲染为地图标记的标签，而不是实体的名称。此选项不适用于[区域](/home-assistant/integrations/zone/)实体，因为它们使用图标而不是标签。
  type: string
attribute:
  required: false
