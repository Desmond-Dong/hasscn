---
title: 自动化模式
description: '自动化可以在正在运行时被再次触发。 本页属于 Home Assistant 中文文档，适合查阅安装部署、集成、自动化与日常使用说明。'
---
# 自动化模式
...
当超过 `max` 时（对于 `single` 模式实际上为 1），将发出日志消息以指示发生了这种情况。配置选项 `max_exceeded` 控制该日志消息的严重级别。将其设置为 `silent` 以忽略警告，或设置为[日志级别](/home-assistant/integrations/logger/#log-levels)。默认值为 `warning`。
