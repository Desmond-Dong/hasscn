---
title: 标签
description: 'Home Assistant 中的标签允许对元素进行分组，而不受其物理位置或类型的限制。标签可以分配给区域、设备、实体、自动化、场景、脚本和助手。标签可以在自动化和脚本中用作动作的目标。标签也可以用于筛选数据。 本页属于 Home Assistant 中文文档，适合查阅安装部署、集成、自动化与日常使用说明。'
---
# 标签
