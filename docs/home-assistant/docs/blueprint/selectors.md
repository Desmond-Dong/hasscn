---
title: "选择器"
description: '选择器可用于指定蓝图输入接受哪些值。选择器还会定义该输入在用户界面中的显示方式。 本页属于 Home Assistant 中文文档，适合查阅安装部署、集成、自动化与日常使用说明。'
---
# 选择器
...
区域选择器还可以根据分配到这些区域中的设备和实体属性来过滤区域列表。例如，可将区域列表限制为仅显示具有 [ZHA](/home-assistant/integrations/zha) 集成提供实体的区域。
...
device:
  ...
  keys:
    integration:
      description: >
        可设置为某个集成域。会将区域列表限制为提供该集成域设备的区域，例如 [`zha`](/home-assistant/integrations/zha)。
      type: string
      required: false
    manufacturer:
      description: >
...
entity:
  ...
  keys:
    integration:
      description: >
        可设置为某个集成域。会将区域列表限制为提供该集成域实体的区域，例如 [`zha`](/home-assistant/integrations/zha)。
      type: string
      required: false
    domain:
      description: >
        将区域列表限制为提供特定[域](/home-assistant/docs/configuration/entities_domains/#domains)实体的区域，例如 [`light`](/home-assistant/integrations/light) 或 [`binary_sensor`](/home-assistant/integrations/binary_sensor)。可以是单个字符串，也可以是字符串列表。
      type: [string, list]
      required: false
    device_class:
      description: >
...
以下示例的区域选择器只会显示提供一个或多个灯或开关实体，且这些实体由 [ZHA](/home-assistant/integrations/zha) 集成提供的区域。
...
下面这个示例使用区域选择器，只显示提供一个或多个遥控器，且这些遥控器由 [deCONZ](/home-assistant/integrations/deconz) 集成提供的区域。您还可以选择多个区域。
...
entity:
  ...
  keys:
    integration:
      description: >
        可设置为某个集成域。会将设备列表限制为提供该集成域实体的设备，例如 [`zha`](/home-assistant/integrations/zha)。
      type: string
      required: false
    domain:
      description: >
        将设备列表限制为提供特定[域](/home-assistant/docs/configuration/entities_domains/#domains)实体的设备，例如 [`light`](/home-assistant/integrations/light) 或 [`binary_sensor`](/home-assistant/integrations/binary_sensor)。可以是单个字符串，也可以是字符串列表。
      type: string
      required: false
    device_class:
      description: >
...
- 由 [deCONZ](/home-assistant/integrations/deconz) 集成提供
- 是型号为 RWL021 的 Philips Hue 遥控器
- 提供一个电池 [sensor](/home-assistant/integrations/sensor)
...
filter:
  ...
  keys:
    integration:
      description: >
        可设置为某个集成域。会将实体列表限制为由该集成域提供的实体，例如 [`zha`](/home-assistant/integrations/zha)。
      type: string
      required: false
    domain:
      description: >
        将实体列表限制为特定[域](/home-assistant/docs/configuration/entities_domains/#domains)中的实体，例如 [`light`](/home-assistant/integrations/light) 或 [`binary_sensor`](/home-assistant/integrations/binary_sensor)。可以是单个字符串，也可以是字符串列表。
      type: [string, list]
      required: false
    device_class:
      description: >
...
- 由 [ZHA](/home-assistant/integrations/zha) 集成提供
- 来自[二元传感器](/home-assistant/integrations/binary_sensor)域
...
楼层选择器可以根据分配到这些楼层中区域的设备和实体属性来过滤楼层列表。例如，可基于所在区域，将楼层列表限制为仅显示具有 [ZHA](/home-assistant/integrations/zha) 集成提供实体的楼层。
...
device:
  ...
  keys:
    integration:
      description: >
        可设置为某个集成域。会将楼层列表限制为包含该集成域设备的楼层，例如 [`zha`](/home-assistant/integrations/zha)。
      type: string
      required: false
    manufacturer:
      description: >
...
entity:
  ...
  keys:
    integration:
      description: >
        可设置为某个集成域。会将楼层列表限制为包含该集成域实体的楼层，例如 [`zha`](/home-assistant/integrations/zha)。
      type: string
      required: false
    domain:
      description: >
        设置后，列表只会包含具有某些[域](/home-assistant/docs/configuration/entities_domains/#domains)实体的楼层，例如 [`light`](/home-assistant/integrations/light) 或 [`binary_sensor`](/home-assistant/integrations/binary_sensor)。可以是单个字符串，也可以是字符串列表。
      type: [string, list]
      required: false
    device_class:
      description: >
...
下面这个楼层选择器只会显示具有一个或多个灯或开关实体，且这些实体由 [ZHA](/home-assistant/integrations/zha) 集成提供的楼层。
...
下面这个示例使用楼层选择器，只会显示具有一个或多个遥控器，且这些遥控器由 [deCONZ](/home-assistant/integrations/deconz) 集成提供的楼层。您还可以选择多个楼层。
...
entity:
  ...
  keys:
    integration:
      description: >
        可设置为某个集成域。会将目标限制为由该集成域提供的实体，例如 [`zha`](/home-assistant/integrations/zha)。
      type: string
      required: false
    domain:
      description: >
        将目标限制为特定[域](/home-assistant/docs/configuration/entities_domains/#domains)中的实体，例如 [`light`](/home-assistant/integrations/light) 或 [`binary_sensor`](/home-assistant/integrations/binary_sensor)。可以是单个字符串，也可以是字符串列表。
      type: [string, list]
      required: false
    device_class:
      description: >
...
下面这个目标选择器示例只会显示至少提供一个或多个灯实体，且这些实体由 [ZHA](/home-assistant/integrations/zha) 集成提供的目标。
