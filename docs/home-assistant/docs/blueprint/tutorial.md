---
title: 创建自动化蓝图
description: '虽然本教程仅展示如何创建自动化蓝图，但脚本也以同样的方式支持蓝图。 本页属于 Home Assistant 中文文档，适合查阅安装部署、集成、自动化与日常使用说明。'
---
# 创建自动化蓝图
...
- 本教程假设您了解以下主题：
  - [编辑配置文件](/home-assistant/docs/configuration/)
- [YAML](/home-assistant/docs/configuration/yaml/)，特别是[自动化中使用的 YAML](/home-assistant/docs/automation/yaml/)
  - [脚本](/home-assistant/docs/scripts/)
...
`trigger` 对象可使用的选项列在[自动化触发器变量](/home-assistant/docs/automation/templating/#available-trigger-data)下。
在此示例中，使用了[状态触发器](/home-assistant/docs/automation/templating/#state)。
`turn_on` 和 `turn_off` 是 [`homeassistant` 动作](/home-assistant/docs/scripts/perform-actions/#homeassistant-actions)。它们不绑定到特定域。您可以在灯光、开关和其他域上使用它们。
...
蓝图中可配置的部分称为[输入](/home-assistant/docs/blueprint/schema/#blueprint-inputs)。为了使运动传感器实体可配置，我们将实体 ID 替换为自定义 YAML 标签 `!input`。此 YAML 标签必须与输入名称结合使用：
...
有关蓝图输入的更多信息，请参阅[蓝图模式文档](/home-assistant/docs/blueprint/schema/#input)
...
为了让 Home Assistant 能够提供更多帮助，我们将使用[选择器](/home-assistant/docs/blueprint/selectors/)。选择器描述了一种类型，可用于帮助用户选择匹配的值。
