...
- 检查应用内 changelog（`app/src/main/res/xml/changelog_master.xml`）是否为最新，并且其版本与正在提升的 beta 相匹配。版本递增通过 [`prepareNextRelease.yml` 工作流](/developers/android/ci#on-pre-release-or-monthly-tag) 创建的 PR 自动进行，但内容应手动验证。
...
- 取消勾选 pre-release 复选框会自动触发 [`release.yml` 工作流](/developers/android/ci#releases)，该工作流使用 Fastlane 在 Play Store 上将 beta 轨道提升到移动端、Wear OS 和 Automotive 应用的生产环境。
...
- F-Droid 会在几天后通过从 GitHub 发布构建来自动获取该发布（参见 [F-Droid 上的发布](/developers/android/ci#release-on-f-droid)）。
