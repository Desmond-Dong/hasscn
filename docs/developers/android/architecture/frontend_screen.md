...
Frontend（JavaScript）和原生代码通过 JavaScript bridge 进行通信。这就是驱动 [external authentication](/developers/frontend/external-authentication) 和 [external bus](/developers/frontend/external-bus) 消息传递的基础。`FrontendScreen` 是唯一持有 WebView 的地方；它下面的所有代码都保持 UI 无关，并通过 `Flow` 通信。
