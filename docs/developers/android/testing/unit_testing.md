...
当你需要访问类的私有部分进行测试时，考虑使用 [VisibleForTesting](https://developer.android.com/reference/kotlin/androidx/annotation/VisibleForTesting) 注解。此注解允许你仅出于测试目的暴露私有方法或属性。[linter](/developers/android/linter) 确保此暴露仅限于测试作用域。
...
- 确保被测代码不依赖于 Android API 的状态，因为这样可能导致不可靠的测试。如果情况如此，考虑编写[instrumented test](/developers/android/testing/integration_testing)。
