---
title: "安卓截图测试"
description: '屏幕截图测试用于验证当前 UI 是否与存储库中存储的参考 UI 匹配。通过这样做，我们可以确保影响 UI 的任何更改都是有意为之且经过验证的。这些测试的范围仅限于 UI。 本页属于 Home Assistant 开发者文档，适合查阅集成、前端、系统、语音与 API 相关实现说明。'
sidebar_label: "截图测试"
---
# 安卓截图测试
...
- **无需模拟器**：这些测试不需要模拟器，因此比[integration tests](/developers/android/testing/integration_testing) 占用的资源更少，并且速度明显更快。
...
我们的[CI pipeline](/developers/android/ci) 验证测试报告是否有任何错误。如果发现差异，CI 会阻止拉取请求，直到问题得到解决。
