...
这里的说明适用于开发 `hassio` 集成。我们假设你已经搭建好 [Home Assistant Core 开发环境](/developers/development_environment)，并且已经配置好 [Supervisor API Access](#supervisor-api-access)。
...
Home Assistant 前端通过 core proxy 使用 supervisor。请参阅 [Home Assistant frontend development environment](/developers/frontend/development) 了解如何进行前端开发。
