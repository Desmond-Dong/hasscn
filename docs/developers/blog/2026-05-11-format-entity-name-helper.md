...
Frontend 还提供了一个 `entity_name` selector。如果你的卡片使用了 [内置 form editor](/developers/frontend/custom-ui/custom-card#using-the-built-in-form-editor)，你可以向用户提供与内置卡片相同的名称选择器——接受自由格式的字符串或 registry items 的组合。

查看更新后的 [data 文档](/developers/frontend/data#hassformatentitynamestateobj-name-options)，了解完整的参考和更多示例。
