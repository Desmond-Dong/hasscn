...
title: "Config Entries 现在可以提供 reconfigure 步骤"
---
...
Reconfiguration 由用户从 config entry 的选项菜单启动，应实现用于更新集成正常工作所必需的、非可选的 config entry 数据。认证问题通过 re-authentication flow 处理。（[查看 reauthentication](/developers/core/integration/config_flow#reauthentication)）。
