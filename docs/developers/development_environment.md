...
请按照我们的[devcontainer 开发环境](/developers/setup_devcontainer_environment)指南，先设置好合适的开发环境。
...
2. 复制你的 fork 的 URL 并粘贴到下方，然后点击 **Open**：
   <RepositoryOpener />
...
_如果你不想使用 devcontainers，才需要这些说明。_
...
:::tip
如果你发现在使用 WSL 时无法通过 [http://localhost:8123](http://localhost:8123) 访问开发实例，请改在 WSL 终端中运行 `ip addr show eth0` 查找 `eth0` 适配器的 `inet` 地址。然后使用该地址（排除 CIDR 块）来访问开发实例，例如，如果你的 `inet` 显示为 `172.20.37.6/20`，则使用 [http://172.20.37.6:8123](http://172.20.37.6:8123)。
:::
