...
- 签署已发布的镜像（支持的 [publishing workflow](/developers/apps/publishing) 可以使用 Cosign 签署镜像）

## 使用 Home Assistant 用户后端

与其允许用户以明文配置设置新的登录凭据，不如使用 Home Assistant [Auth backend](/developers/api/supervisor/endpoints#auth)。你可以通过 `auth_api: true` 启用对 API 的访问。现在你可以将登录凭据发送到 auth backend，并在 Home Assistant 中进行验证。
