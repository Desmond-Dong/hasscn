...
|软件版本|设备 的固件版本。|
|via_设备|在该设备与 Home Assistant 之间路由消息的设备标识符。此类设备的例子包括集线器或子设备的父设备。这用于展示 Home Assistant 中的设备拓扑。|

## 定义设备

### 通过实体自动注册
:::tip
仅当通过[配置条目](/developers/config_entries_index)加载实体，并且定义了 `unique_id` 属性时，才会读取实体设备信息。
:::
