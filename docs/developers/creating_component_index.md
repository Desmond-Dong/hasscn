...
:::tip
这个示例仓库展示了存放在 `<config_dir>/custom_components` 目录中的自定义集成。它们必须在[清单文件](/developers/creating_integration_manifest#version)中包含 `version` 键。核心集成存放在 `homeassistant/components` 目录中，不需要 `version` 键。两种情况下的架构是相同的。
:::
