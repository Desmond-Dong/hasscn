...
该仓库的组织方式尽可能在各目标间共享逻辑，同时允许在需要时使用平台特定实现。数据库访问、网络、设计系统组件、通知、widgets 和共享模型等跨领域关注点都放在 common 模块中，以便多个目标可以复用它们。请参阅[目标指南](/developers/apple/targets)，了解每个 surface 的概述。
