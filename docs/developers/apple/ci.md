...
每次 pull request，CI 运行 [编码风格指南](/developers/apple/codestyle) 中描述的相同 linter：
...
CI 验证的不仅仅是编译：风格、字符串和 artifact 也是审查循环的一部分。有关 linting，请参阅[编码风格指南](/developers/apple/codestyle)；有关在推送之前在本地运行测试，请参阅[入门指南](/developers/apple/get_started)。
