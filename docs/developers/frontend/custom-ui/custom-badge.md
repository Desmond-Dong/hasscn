...
在你的 dashboard 配置中添加一个资源，URL 为 `/local/text-badge.js`，类型为 `module`（[resource docs](/developers/frontend/custom-ui/registering-resources)）。
...
当 Home Assistant 状态发生变化时（很频繁），Home Assistant 会设置[`hass` 属性](/developers/frontend/data)。每当状态变化时，组件都必须更新自身以反映最新状态。
