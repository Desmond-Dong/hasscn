...
Strategies 在 JavaScript 文件中定义为 custom element，并通过[dashboard resources](/developers/frontend/custom-ui/registering-resources)引入。Home Assistant 不会将其渲染为 custom element，而是会调用类上的静态函数。
