...
### 1. External requirements
 ...
 1. 依赖项已添加到 [`manifest.json`](creating_integration_manifest.md)。`REQUIREMENTS` 常量已弃用。
 2. 依赖项版本必须固定：`"requirements": ['phue==0.8.1']`
 4. 每个依赖项都满足[库要求](api_lib_index.md#basic-library-requirements)。

### 2. Configuration

1. 有用于[配置验证](development_validation.md) 的 Voluptuous schema
