...
| code_format | string | None | 代码格式的正则表达式或无（如果不需要代码）。
| is_locked | bool | None | 指示锁当前是否已锁定。用于确定 `state`。
| is_locking | bool | None | 指示锁当前是否处于锁定状态。用于确定 `state`。
| is_unlocking | bool | None | 指示锁当前是否正在解锁。用于确定 `state`。
| is_jammed | bool | None | 指示锁当前是否被卡住。用于确定 `state`。
| is_opening | bool | None | 指示锁当前是否正在打开。用于确定 `state`。
