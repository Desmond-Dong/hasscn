...
| ---- | ---- | ------- | -----------
| alarm_state | `AlarmControlPanelState \| None` | **必需** | **States** 中列出的 alarm 值之一。
| code_arm_required | bool | `True` | 执行 arm actions 时是否需要 code。
| code_format | `CodeFormat \| None` | `None` | **Code formats** section 中列出的 states 之一。
| changed_by | `str \| None` | `None` | 上次变更的触发者。
