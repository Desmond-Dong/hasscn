...
| ---- | ---- | ------- | -----------
| is_on| `bool \| None` | `None` | **必需**。Binary sensor 当前是 on 还是 off。
| device_class | `BinarySensorDeviceClass \| None` | `None` | Binary sensor 的类型。
...
| `BinarySensorDeviceClass.UPDATE` | On 表示有 update 可用，Off 表示已是最新。应避免使用此 device class，请考虑改用 [`update`](/developers/core/entity/update) entity。
