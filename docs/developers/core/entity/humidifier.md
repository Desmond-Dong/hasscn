...
| available_modes         | `list[str] \| None`             | **MODES 必填**                 | 可用的模式。需要 `HumidifierEntityFeature.MODES`。     |
| current_humidity        | `float \| None`                   | `None`                                | 设备测量的当前湿度。       |
| device_class            | `HumidifierDeviceClass \| None` | `None`                                | hygrostat 的类型                                  |
| is_on                   | `bool \| None`                  | `None`                                | 设备是开启还是关闭。                   |
...
| target_humidity         | `float \| None`                   | `None`                                | 设备试图达到的目标湿度。 |
| target_humidity_step    | `float \| None`                   | `None`                                | 目标湿度可增加或减少的步长。 |
