...
如果你正在使用 [DataUpdateCoordinator](/developers/integration_fetching_data.md#coordinated-single-api-poll-for-data-for-all-entities)，调用 `await coordinator.async_config_entry_first_refresh()` 也会在首次刷新失败时自动触发此异常。
...
* source：始终为 "SOURCE\_REAUTH"
* entry\_id：需要重新认证的 config entry 的 entry\_id
* unique\_id：需要重新认证的 config entry 的 unique\_id
