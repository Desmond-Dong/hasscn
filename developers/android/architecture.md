...
Wear OS 应用程序与移动应用程序通信，以使用 [Messaging API](https://developer.android.com/training/wearables/data/messages) 搜索 Home Assistant 服务和其他配置的凭据。它仅适用于 `full` 风格，因为最终的 Google Play 服务。设置完成后，所有后续的通信都将通过 WebSocket 并为应用程序创建的 [webhook](/developers/api/native-app-integration/sending-data.md) 直接与 Home Assistant 处理。
