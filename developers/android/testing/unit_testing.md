...
当您需要访问类的私有部分进行测试时，请考虑使用[VisibleForTesting](https://developer.android.com/reference/kotlin/androidx/annotation/VisibleForTesting)注释。此注释允许您公开私有方法或属性，仅用于测试目的。 [linter](/developers/android/linter.md) 确保这种暴露仅限于测试范围。
...
* 确保您正在测试的代码不依赖于 Android API 的状态，因为这可能会导致测试不可靠。如果是这种情况，请考虑编写[instrumented test](/developers/android/testing/integration_testing.md)。
