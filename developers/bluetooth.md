...
* 需要使用 Bluetooth 的集成，应在其 [`manifest.json`](/developers/creating_integration_manifest.md) 的 [`dependencies`](/developers/creating_integration_manifest.md#dependencies) 中添加 `bluetooth_adapters`。这样可以确保在集成尝试使用所有受支持的远程适配器之前，这些适配器已经完成连接。
