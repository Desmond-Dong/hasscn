...
* 需要使用蓝牙适配器的集成应在 [`manifest.json`](/developers/creating_integration_manifest.md) 中的 [`dependencies`](/developers/creating_integration_manifest.md#dependencies) 添加 `bluetooth_adapters`。[`manifest.json`](/developers/creating_integration_manifest.md) 条目可确保所有受支持的远程适配器在集成尝试使用它们之前均已连接。
