...
在使用这些 helpers 之前，请确保在你的 integration 的 [`manifest.json`](/developers/creating_integration_manifest.md) 中将 `zeroconf` 添加到 `dependencies` 中。
...
在使用这些 helpers 之前，请确保在你的 integration 的 [`manifest.json`](/developers/creating_integration_manifest.md) 中将 `ssdp` 添加到 `dependencies` 中。
...
某些 integrations 可能需要立即获知设备被发现的事件。SSDP integration 提供了一个 registration API，当发现的新设备匹配特定 key values 时接收回调。匹配规则使用与 [`manifest.json`](/developers/creating_integration_manifest.md) 中 `ssdp` 相同的格式。
...
在使用这些 helpers 之前，请确保在你的 integration 的 [`manifest.json`](/developers/creating_integration_manifest.md) 中将 `dhcp` 添加到 `dependencies` 中。
