...
* 不要使用 "e.g."。改用 *for example*、*such as* 或 *like*。
...
* 品牌名、服务名、协议名、集成名和平台名的大小写必须与其官方对应。例如，"Z-Wave"，*不是* "Zwave"、"Z-wave"、"Z Wave" 或 "ZWave"。同样，"Input Select"，*不是* "input select" 或 "Input select"。
...
添加集成时关于图片的内容，请参见 [Images, icons, and logos](/developers/documenting/create-page.md#images-icons-and-logos)。
...
关于编写集成文档时有用的示例，请参见 [Configuration](/developers/documenting/create-page.md#configuration)。
...
关于 glossary 用法的更多推荐信息，请参见 [Glossary & terminology tooltips](/developers/documenting/standards.md#glossary--terminology-tooltips)。
