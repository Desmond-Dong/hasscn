...
这是一个 voluptuous schema，用于验证某个特定的触发器字典是否表示你的集成可以处理的配置。它应该扩展自 `device_automation/__init__.py` 中的 TRIGGER\_BASE\_SCHEMA。
...
例如，你可以将触发器和 action 关联到你的集成在 event bus 上触发的[事件](/developers/integration_events.md)。
