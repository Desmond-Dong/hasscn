...
:::info
Service actions 的名称和描述在我们的 [translations](/developers/internationalization/core.md) 中设置，而不是在 service action description 中。每个 service action 和 service action field 都必须有对应的 translation 定义。Description placeholders 允许你排除 URL 等元素而不翻译。
...
注册 service action 时，应将其指向 [data hierarchy](/developers/architecture/devices-and-services.md#entity-data-hierarchy) 中该 action 真正需要运行的层级。不要指向比所需层级更高或更低的层级，即使这些层级可以互相解析。
