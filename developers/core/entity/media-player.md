...
| 名称 | 类型 | 默认值 | 说明
| ------------------------------- | ----------------------------------------------- | ------- | -----------
| app\_id | <code>str | None</code> | `None` | 当前运行的应用程序的ID。
| app\_name | <code>str | None</code> | `None` | 当前运行的应用程序的名称。
| device\_class | <code>MediaPlayerDeviceClass | None</code> | `None` | 媒体播放器的类型。
| group\_members | <code>list\[str] | None</code> | `None` | 当前分组在一起以进行同步播放的播放器实体的动态列表。如果平台有定义组领导者的概念，则领导者应该是该列表中的第一个元素。
| is\_volume\_muted | <code>bool | None</code> | `None` | `True` 如果音量当前处于静音状态。
| media\_album\_artist | <code>str | None</code> | `None` | 当前播放媒体的专辑艺术家，仅限音乐曲目。
| media\_album\_name | <code>str | None</code> | `None` | 当前播放媒体的专辑名称，仅音乐曲目。
| media\_artist | <code>str | None</code> | `None` | 仅当前播放媒体、音乐曲目的艺术家。
| media\_channel | <code>str | None</code> | `None` | 当前正在播放的频道。
| media\_content\_id | <code>str | None</code> | `None` | 当前播放媒体的内容 ID。
| media\_content\_type | <code>MediaType | str | None</code> | `None` | 当前播放媒体的内容类型。
| media\_duration | <code>int | None</code> | `None` | 当前播放媒体的持续时间（以秒为单位）。
| media\_episode | <code>str | None</code> | `None` | 当前播放媒体的剧集，仅限电视节目。
| media\_image\_hash | <code>str | None</code> | `None` | 媒体图像的哈希值，如果 `media_image_url` 不是 `None`，则默认为 `media_image_url` 的 SHA256。
| media\_image\_remotely\_accessible | <code>bool | None</code> | `False` | `True`（如果可以在家庭网络外部访问属性 `media_image_url`）。
| media\_image\_url | <code>str | None</code> | `None` | 当前播放媒体的图片URL。
| media\_playlist | <code>str | None</code> | `None` | 当前播放的播放列表的标题。
| media\_position | <code>int | None</code> | `None` | 当前播放媒体的位置（以秒为单位）。
| media\_position\_updated\_at | <code>datetime | None</code> | `None` | `_attr_media_position` 上次更新的时间戳。应通过调用 `homeassistant.util.dt.utcnow()` 设置时间戳。
| media\_season | <code>str | None</code> | `None` | 当前播放的媒体季，仅限电视节目。
| media\_series\_title | <code>str | None</code> | `None` | 当前播放媒体、电视节目的系列标题。
| media\_title | <code>str | None</code> | `None` | 当前播放媒体的标题。
| media\_track | <code>int | None</code> | `None` | 当前播放媒体的曲目编号，仅限音乐曲目。
| repeat | <code>RepeatMode | str | None</code> | `None` | 当前重复模式。
| shuffle | <code>bool | None</code> | `None` | 如果启用随机播放，则为 `True`。
| sound\_mode | <code>str | None</code> | `None` | 媒体播放器当前的声音模式。
| sound\_mode\_list | <code>list\[str] | None</code> | `None` | 可用声音模式的动态列表。
| source | <code>str | None</code> | `None` | 当前为媒体播放器选择的输入源。
| source\_list | <code>list\[str] | None</code> | `None` | 媒体播放器可能的输入源列表。 （此列表应包含人类可读的名称，适合前端显示）。
| state | <code>MediaPlayerState | None</code> | `None` | 媒体播放器的状态。
| volume\_level | <code>float | None</code> | `None` | 媒体播放器的音量级别在 (0..1) 范围内。
| volume\_step | <code>float | None</code> | 0.1 | 用于 `volume_up` 和 `volume_down` 维修操作的音量步骤。
...
媒体播放器在待机状态下无法控制的情况很常见。如果 Home Assistant 可以使用其他协议或方法打开设备，即使用于控制设备的主通道当前不可用，它也应显示为 `off`。如果 Home Assistant 无法打开设备，则应显示为 `unavailable`。有关更多详细信息，请参阅[实体不可用的异常](/developers/core/integration-quality-scale/rules/entity-unavailable.md#Exceptions)。
