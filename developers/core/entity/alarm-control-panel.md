...
| 名称 | 类型 | 默认值 | 说明
| ---- | ---- | ------- | -----------
| alarm\_state | <code>AlarmControlPanelState | None</code> | **Required** | **州** 中列出的警报值之一。
| code\_arm\_required | bool | `True` | 手臂动作是否需要该代码。
| code\_format | <code>CodeFormat | None</code> | `None` | **代码格式** 部分中列出的状态之一。
| changed\_by | <code>str | None</code> | `None` | 最后一次更改触发的。
