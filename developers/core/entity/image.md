...
图像实体是[`camera`](/developers/core/entity/camera.md)实体的简化版本，用于提供静态图像或可拉取的图像 URL。
...
| 名称 | 类型 | 默认值 | 说明 |
| -------------------| --------------------------------- | ------------ | -------------------------------------------------------------------------------------------------------- |
| content\_type | str | `image/jpeg` | 图像的内容类型，如果图像实体提供 URL，则自动设置。 |
| image\_last\_updated | <code>datetime.datetime | None</code> | `None` | 上次更新图像的时间戳。用于确定 `state`。更改后前端将调用 `image` 或 `async_image`。 |
| image\_url | <code>str | None</code> | `UNDEFINED` | 可选。用于获取图像的 URL。 |
