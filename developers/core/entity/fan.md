...
| ---- | ---- | ------- | -----------
| current\_direction  | `str \| None`       | `None` | Fan 当前的 direction。 |
| is\_on              | `bool \| None`      | `None` | 如果 fan 已开启则为 `True`。 |
| oscillating        | `bool \| None`       | `None` | 如果 fan 正在 oscillating 则为 `True`。 |
| percentage         | `int \| None`       | `0`    | 当前的 speed percentage。必须是 0（off）到 100 之间的值。 |
| preset\_mode        | `str \| None`       | `None` | 当前的 preset\_mode。为 `preset_modes` 中的值之一，或如果没有活跃的 preset 则为 `None`。 |
| preset\_modes       | `list[str] \| None` | `None` | 支持的 preset\_modes 列表。这是一个任意 str 列表，不应包含任何 speed。 |
| speed\_count        | `int`               | 100    | Fan 支持的 speed 数量。 |
