...
| 名称 | 类型 | 默认值 | 说明 |
| ----------------------- | ----------------------------------- | ------------------------------------ | -------------------------------------------------------------------------- |
| current\_humidity | <code>float | None</code> | `None` | 当前湿度。 |
| current\_temperature | <code>float | None</code> | `None` | 当前温度。 |
| fan\_mode | <code>str | None</code> | **Required by SUPPORT\_FAN\_MODE** | 当前风扇模式。 |
| fan\_modes | <code>list\[str] | None</code> | **Required by SUPPORT\_FAN\_MODE** | 可用风扇模式列表。 |
| hvac\_action | <code>HVACAction | None</code> | `None` | 当前的 HVAC 操作（加热、冷却） |
| hvac\_mode | <code>HVACMode | None</code> | **Required** | 当前操作（例如加热、冷却、空闲）。用于确定 `state`。 |
| hvac\_modes | <code>list\[HVACMode]</code> | **Required** | 可用操作模式列表。见下文。 |
| max\_humidity | `float` | `DEFAULT_MAX_HUMIDITY` (value == 99) | 最大湿度。 |
| max\_temp | `float` | `DEFAULT_MAX_TEMP` (value == 35 °C) | 最高温度为 `temperature_unit`。 |
| min\_humidity | `float` | `DEFAULT_MIN_HUMIDITY` (value == 30) | 最低湿度。 |
| min\_temp | `float` | `DEFAULT_MIN_TEMP` (value == 7 °C) | 最低温度为 `temperature_unit`。 |
| precision | `float` | According to `temperature_unit` | 系统内温度的精度。 TEMP\_CELSIUS 默认为十分之一，否则为整数。 |
| preset\_mode | <code>str | None</code> | **Required by SUPPORT\_PRESET\_MODE** | 当前活动预设。 |
| preset\_modes | <code>list\[str] | None</code> | **Required by SUPPORT\_PRESET\_MODE** | 可用的预设。 |
| swing\_mode | <code>str | None</code> | **Required by SUPPORT\_SWING\_MODE** | 摆动设置。 |
| swing\_modes | <code>list\[str] | None</code> | **Required by SUPPORT\_SWING\_MODE** | 返回可用摆动模式的列表，仅在实现水平摆动的情况下为垂直模式。 |
| swing\_horizontal\_mode | <code>str | None</code> | **Required by SUPPORT\_SWING\_HORIZONTAL\_MODE** | 水平摆动设置。 |
| swing\_horizontal\_modes | <code>list\[str] | None</code> | **Required by SUPPORT\_SWING\_HORIZONTAL\_MODE** | 返回可用水平摆动模式的列表。 |
| target\_humidity | <code>float | None</code> | `None` | 设备试图达到的目标湿度。 |
| target\_humidity\_step | <code>int | None</code> | `None` | 可以在针对设备的操作调用中增加或减少目标湿度支持的步长。 |
| target\_temperature | <code>float | None</code> | `None` | 当前设定的要达到的温度。 |
| target\_temperature\_high | <code>float | None</code> | **Required by TARGET\_TEMPERATURE\_RANGE** | 上限目标温度 |
| target\_temperature\_low | <code>float | None</code> | **Required by TARGET\_TEMPERATURE\_RANGE** | 下限目标温度 |
| target\_temperature\_step | <code>float | None</code> | `None` | 可以增加或减少目标温度支持的步长 |
| temperature\_unit | <code>str</code> | **Required** | 系统温度测量单位（`TEMP_CELSIUS` 或 `TEMP_FAHRENHEIT`）。 |
...
HVAC 操作描述了\_current\_ 操作。这与模式不同，因为如果将设备设置为加热，并且已经达到目标温度，则设备将不再主动加热。仅允许使用 `HVACAction` 枚举提供的内置 HVAC 操作。
