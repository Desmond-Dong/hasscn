...
| 名称                 | 类型                   | 默认值           | 描述                                                           |
|----------------------|------------------------|-------------------|-----------------------------------------------------------------------|
| `pipeline_entity_id` | `str \| None` | `None` | 带有 [pipeline id](/developers/voice/pipelines.md) 的 `select` 实体的 Id，或 `None`。 |
...
`AssistSatelliteEntity` 的状态跟随其当前正在运行的 [pipeline](/developers/voice/pipelines.md)。`AssistSatelliteState` 枚举存储了可能的状态。
...
Satellite 实体应仅使用 `async_accept_pipeline_from_satellite` 方法运行 [Assist pipeline](/developers/voice/pipelines.md)。通过实现 `on_pipeline_event` 方法来处理 [来自 pipeline 的事件](/developers/voice/pipelines.md#events)。
