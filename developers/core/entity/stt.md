...
| 名称 | 类型 | 默认值 | 描述
| ---- | ---- | ------- | -----------
| supported\_languages | list\[str] | **必填** | STT 服务支持的语言。
| supported\_formats | list\[AudioFormats] | **必填** | STT 服务支持的音频格式，wav 或 ogg。
| supported\_codecs | list\[AudioCodecs] | **必填** | STT 服务支持的音频编解码器，pcm 或 opus。
| supported\_bit\_rates | list\[AudioBitRates] | **必填** | STT 服务支持的音频比特率，8、16、24 或 32。
| supported\_sample\_rates | list\[AudioSampleRates] | **必填** | STT 服务支持的音频采样率。
| supported\_channels | list\[AudioChannels] | **必填** | STT 服务支持的音频声道，1 或 2。
