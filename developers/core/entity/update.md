...
| ---- | ---- | ------- | -----------
| auto\_update | bool | `False` | Entity 所代表的 device 或 service 具有 auto update logic。当此值设为 `True` 时，不能跳过 updates。
| display\_precision | int | `0` | 显示 update progress 的小数位数。
| in\_progress | bool | `False` | Update installation progress。应返回布尔值（进行中标记为 True，未进行标记为 False）。
| installed\_version | str | `None` | 当前已安装并使用的 software version。
| latest\_version | str | `None` | 可用 software 的最新 version。
| release\_summary | str | `None` | Release notes 或 changelog 的摘要。不适合长 changelogs，仅适合最多 255 字符的简短 update description 摘录。
| release\_url | str | `None` | 可用最新 version 的完整 release notes 的 URL。
| title | str | `None` | Software 的 title。这有助于区分 device 或 entity name 与已安装 software 的 title。
| update\_percentage | int, float | `None` | Update installation progress。可以返回一个 0 到 100% 的 progress 数字，或 None。
