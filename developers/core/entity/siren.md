...
| 名称 | 类型 | 默认值 | 说明 |
| ----------------------- | ------ | ------------------------------------- | --------------------------------------------------------------------------------------- |
| is\_on | bool | `None` | 设备是否打开或关闭。 |
| available\_tones | list or dict | `NotImplementedError()` | 设备上要传递到 `turn_on` 服务操作的可用提示音的列表或字典。如果提供字典，当用户使用语气的字典值时，它会在传递到集成平台之前转换为相应的字典键。需要 `SUPPORT_TONES` 功能。 |
