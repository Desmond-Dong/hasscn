...
| available\_modes | <code>list\[str] | None</code> | **Required by MODES** | 可用的模式。需要 `SUPPORT_MODES`。 |
| current\_humidity | <code>float | None</code> | `None` | 设备测量的当前湿度。 |
| device\_class | <code>HumidifierDeviceClass | None</code> | `None` | 恒湿器类型 |
| is\_on | <code>bool | None</code> | `None` | 设备是否打开或关闭。 |
| max\_humidity | `float` | `DEFAULT_MAX_HUMIDITY` (value == 100) | 最大湿度。 |
| min\_humidity | `float` | `DEFAULT_MIN_HUMIDITY` (value == 0) | 最低湿度。 |
| mode | <code>str | None</code> | **Required** | 当前活动模式。需要 `SUPPORT_MODES`。 |
| target\_humidity | <code>float | None</code> | `None` | 设备试图达到的目标湿度。 |
| target\_humidity\_step | <code>float | None</code> | `None` | 可以增加或减少目标湿度支持的步长。 |
