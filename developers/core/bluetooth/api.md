...
Bluetooth 集成跟踪每个设备已看到哪些 advertisement 字段（manufacturer\_data UUIDs、service\_data UUIDs、service\_uuids），以确定何时触发 discovery。它只检查 UUIDs 是否曾被看到过，而不检查其内容是否已更改。
...
提供 Bluetooth adapter 的集成应在 `manifest.json` 的 [`dependencies`](/developers/creating_integration_manifest.md#dependencies) 中添加 `bluetooth`，并被添加到 `bluetooth_adapters` 集成的 [`after_dependencies`](/developers/creating_integration_manifest.md#after-dependencies)。
