...
* **使用 `DataUpdateCoordinator` 进行轮询。** 轮询集成不应实现自己的轮询循环。例外情况是集成使用[每个 individual entity 的单独轮询](/developers/integration_fetching_data.md#separate-polling-for-each-individual-entity)。
...
* **使用 Entity description。** 新 entity 应使用基于 dataclass 的 [entity description](/developers/core/entity.md#entity-description)（如 `SensorEntityDescription`）来定义。链接文档中描述的其他模式也被接受，但在新代码中优先使用 entity description 模式。
