...
Home Assistant 具有内置的 API，可将 Assist API 暴露给 LLMs。此 API 允许 LLMs 通过 [intents](/developers/intent_builtin.md) 与 Home Assistant 交互，并可以通过注册 intents 来扩展。
...
| `device_id`       | string  | 用户用来发起对话的 device 的 device\_id。                                                                  |
...
由于这些 endpoints 是通过 API ID 寻址的，clients 需要一种方式来查找哪些 ID 存在。已注册的 APIs 可以通过 [WebSocket API](/developers/api/websocket.md) 列出：
