...
这是一个 voluptuous schema，用于验证某个特定的触发器字典是否代表你的集成可以处理的配置。它应该扩展自 `device_automation/__init__.py` 中的 TRIGGER\_BASE\_SCHEMA。
...
例如，你可以将触发器和动作挂载到你的集成在事件总线上发出的[事件](/developers/integration_events.md)上。
