...
1. 下载 *UMS Utility* 镜像：[`ODROID-M1S_EMMC2UMS.img`][1]。*UMS Utility* 是一个可将 ODROID-M1S 切换为 USB 大容量存储设备的特殊镜像。
2. 使用 balenaEtcher 或其他工具将 *UMS utility* 刷写到 SD 卡。
...
接着，从你的 PC 复制该二进制文件（例如 `ssh user@mypc.local:/path_to/ODROID-M1S_EMMC2UMS.img /tmp` —— 将其中的 user 替换为你在 PC 上的用户名，将 mypc.local 替换为你的计算机名或 IP 地址，将 path\_to 替换为实际的下载路径）。这样会把该二进制文件复制到 HAOS 的 `/tmp/`。
