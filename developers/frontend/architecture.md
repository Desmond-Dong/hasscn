...
前端利用 [WebSocket API](/developers/api/websocket.md) 和 [REST API](/developers/api/rest.md) 与 Home Assistant 交互。

数据通过 `hass` 属性提供，并向下传递给每个组件。`hass` 属性包含核心状态，并拥有调用 API 的方法。

只需要 Home Assistant 部分状态的组件，应改为使用相关的 [Lit context](/developers/frontend/data.md#context)。
