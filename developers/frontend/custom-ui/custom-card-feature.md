...
一些 dashboard cards 支持 [features](https://www.home-assistant.io/dashboards/features/)。这些小部件会为 card 添加快捷控制。我们提供了许多内置 features，但你并不局限于 Home Assistant 默认包含的那些。你可以像定义[custom cards](/developers/frontend/custom-ui/custom-card.md)一样构建并使用自己的 feature。

## 定义你的卡片功能

下面是一个用于 [button entity](/developers/core/entity/button.md) 的 custom card feature 示例。
