...
在你的 dashboard 配置中添加一个 resource，URL 为 `/local/text-badge.js`，type 为 `module`（[resource 文档](/developers/frontend/custom-ui/registering-resources.md)）。
...
Home Assistant 会在 state 变更时（频繁）设置 [the `hass` property](/developers/frontend/data.md)。每当 state 变更时，该组件都必须更新自身以反映最新的 state。
