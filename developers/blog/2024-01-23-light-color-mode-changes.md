...
例如，一个 supported\_color\_modes 设置为 `{"hs", "color_temp"}` 的 light，在渲染无法调节的 effect 时允许将其 `color_mode` 设置为 `on_off`，在渲染允许控制 brightness 的 effect 时允许设置为 `brightness`。
...
更多详情可在[文档](/developers/core/entity/light.md#color-modes)和[架构讨论 #960](https://github.com/home-assistant/architecture/discussions/960)中找到。
