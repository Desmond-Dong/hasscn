...
多年来，`pyserial` 一直是 Python 中的事实标准 serial 库，并对所有流行平台有广泛支持。然而，其 API 早于 Python 中的 asyncio，且仅支持 sync。后来发布了 `pyserial-asyncio` 包以桥接 `pyserial` 与 asyncio。不幸的是，`pyserial-asyncio` 的开发从未达到 1.0 稳定性或跨平台支持，而且 `pyserial` 和 `pyserial-asyncio` 在近五年内都没有 PyPI 发布。我们 fork 了 `pyserial-asyncio` 并发布了 [`pyserial-asyncio-fast`](/developers/blog/2026-01-05-pyserial-asyncio-fast.md)，以修复影响 Core event loop 稳定性的未解决问题。
