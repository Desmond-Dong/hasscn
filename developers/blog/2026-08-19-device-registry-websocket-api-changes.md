...
* **将每个 device 限制为单个 config entry**（Home Assistant Core 2026.8）。此变更的 Python API 方面在 companion post [Devices are restricted to a single config entry and at most one subentry](/developers/blog/2026-07-21-device-registry-single-config-entry.md) 中描述。
...
Companion post 的 [Backwards compatibility](/developers/blog/2026-07-21-device-registry-single-config-entry.md#backwards-compatibility) 部分描述了弃用期间行为的其余部分，例如针对 composite device id 的 actions 级联到拆分 devices。
...
详情请参阅 companion post 中的 [Device registry events](/developers/blog/2026-07-21-device-registry-single-config-entry.md#device-registry-events)。
