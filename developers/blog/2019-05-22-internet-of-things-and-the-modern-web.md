...
* **使用 modules 和 unpkg 轻松进行扩展开发。** 开发者无需运行任何工具即可开发自己的 card。你只需要创建一个 JavaScript 文件，在配置的 resource 部分引用它，就可以开始使用了（[文档中的示例](/developers/frontend/custom-ui/custom-card.md#example-using-js-modules)）。我们默认将资源作为 type=module 导入，这样开发者可以在开发过程中直接从 http://unpkg.com 导入他们的依赖。
