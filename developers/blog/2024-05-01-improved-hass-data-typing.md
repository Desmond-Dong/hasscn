...
按 `entry.entry_id` 将数据存储在 dict 中？通常直接将数据存储到 `ConfigEntry` 中会更好。请参见关于此的近期[博客文章](/developers/blog/2024-04-30-store-runtime-data-inside-config-entry.md)。如果这不是一个选项，请使用 `HassEntryKey`。
