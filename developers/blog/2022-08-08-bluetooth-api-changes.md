...
PassiveBluetoothProcessorCoordinator 现在接收一个必填的 update\_method 回调，该回调接收 bluetooth advertisements（形式为 BluetoothServiceInfoBleak），并返回应移交给任何订阅的 PassiveBluetoothDataProcessor 的数据：
...
PassiveBluetoothDataProcessor 仍然接收一个 update\_method，但它现在接收的不是 BluetoothServiceInfoBleak，而是来自 PassiveBluetoothProcessorCoordinator 的 update\_method 返回的数据。它仍然应该像以前一样返回一个 `PassiveBluetoothDataUpdate`：
