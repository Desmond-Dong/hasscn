这些说明适用于 Supervisor、Supervisor 前端面板和 `hassio` 集成的开发，通过与发展中的或远程的 supervisor 交互进行，这假设你正在使用一台开发机器进行开发。这些说明还将使用 devcontainer 和其他依赖 Docker 的工具，请先按照我们的 [devcontainer 开发环境](/developers/setup_devcontainer_environment.md) 指南设置合适的开发环境。
...
Home Assistant 前端通过核心代理使用 supervisor。请查看 [Home Assistant 前端开发环境](/developers/frontend/development.md) 了解如何开发前端。
