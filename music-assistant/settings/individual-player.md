...
单个播放器设置通常相当广泛。下图显示了各个部分的典型标题，下面的小节将对它们进行扩展。群组有相同的部分，但可用的设置较少。并非所有播放器都列出了所有设置。可以参考[播放器汇总表](/music-assistant/player-support/index.md)来识别特定播放器类型应有的一些功能，否则请访问播放器的设置页面。
...
* <b>在 UI 中隐藏此播放器。</b> 此设置确定何时不在[播放器列表](/music-assistant/ui/index.md#player-list)中显示播放器。选项有 `不可用时`、`群组活动时`、`同步时`、`始终` 和 `关闭时`
* <b>动态成员</b>切换。此设置适用于[同步群组和通用群组](/music-assistant/faq/groups/index.md)。启用后，可以添加和删除这些群组类型的成员
...
* <b>启用音量标准化。</b> 此设置通常应保持启用，以避免来自不同来源的音轨之间音量水平不同。在[技术信息页面](/music-assistant/faq/tech-info/index.md#volume-normalization)阅读更多相关内容。目标级别在[高级设置](#advanced-settings)中设置
...
* [参数均衡器](/music-assistant/dsp/parametriceq/index.md)
* [音调控制](/music-assistant/dsp/tonecontrols/index.md)
...
可以将其他 HA 实体映射到 MA 播放器控制。为了使此成为选项，HA 实体需要首先通过 [HA 插件](/music-assistant/ha-plugin/index.md)中的设置暴露给 MA。
...
* [AirPlay](/music-assistant/player-support/airplay/index.md#settings)
* [Squeezelite](/music-assistant/player-support/squeezelite/index.md#settings)
* [Google Cast](/music-assistant/player-support/google-cast/index.md#settings)

## 高级设置

* <b>[音量标准化](/music-assistant/faq/tech-info/index.md#volume-normalization)的目标级别</b>。-17 的默认设置通常不应更改。如果设置得太高，可能会导致削波。音量标准化在[音频](#audio)部分启用和禁用
...
* <b>音频同步延迟校正</b>。请参阅[播放器汇总表](/music-assistant/player-support/index.md)以识别哪些类型支持同步校正
