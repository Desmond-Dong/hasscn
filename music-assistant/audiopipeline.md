...
接下来显示音量标准化值。有关如何应用音量标准化的详细信息，请参阅[技术信息](/music-assistant/faq/tech-info/index.md#volume-normalization)页面。
...
可以预期的最大采样率可以在[播放器提供程序汇总表](/music-assistant/player-support/index.md)中找到。如果更改了播放器的"此播放器支持的采样率"高级设置的默认选项，则可能会进一步限制。
...
在此示例中，已启用[DSP](/music-assistant/settings/individual-player/index.md#dsp-settings)。显示了已应用的DSP滤波器的高级信息。如果不支持DSP，会有工具提示解释原因（参见下文[群组](#groups)中的示例）。
...
视图会根据需要扩展以显示所有[群组播放器](/music-assistant/faq/groups/index.md)。在上面的示例中，显示了三个播放器，具有各种滤波器、输出质量和播放器类型。
