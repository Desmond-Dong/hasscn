...
* [群组](/music-assistant/faq/groups/index.md)播放使用单个播放器 [DSP 设置](/music-assistant/settings/individual-player/index.md#dsp-settings)
* Squeezelite 客户端设备按钮支持
  * 设备上的任何物理控制按钮都应受支持，只要未启用[流模式](/music-assistant/faq/tech-info/index.md#track-queueing)

## 设置

除了[单个播放器设置](/music-assistant/settings/individual-player/index.md)外，Squeezelite 提供商在高级部分还有一个独特设置和一个独特的预设部分

* <b>高级 - 启用显示支持。</b> 某些 Squeezelite 硬件有显示屏，此设置启用支持。启用后，会提供第二个选项来选择 `可视化类型`
* <b>预设。</b> 真正的 Squeezebox 硬件或基于 jive(lite) 的模拟器支持预设。此部分允许将[播放列表](/music-assistant/usage/index.md#playlists)或广播电台分配给这些预设
