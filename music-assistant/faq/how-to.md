...
您也可以使用 `music_assistant.search` 或 `music_assistant.get_library` 操作，URI 将显示在结果中。URI 也显示在项目视图底部的[提供商详情部分](/music-assistant/ui/index.md#view-individual-artist)中，可以使用链接图标复制到剪贴板。

> \[!NOTE]
> 以 `media-source://` 开头的 URI 是 HA URI，在针对 MA 播放器实体时不应使用。这样做将导致不一致的行为。

文件夹的 URI 需要以 `filesystem_id://folder/relative/path/to/folder` 的形式构造（例如 `filesystem_smb--5iJ4npRi://folder/ABBA`），filesystem\_id 可以通过查看 `get_library` 操作的输出来获取。扫描键 `tracks.provider_mappings.provider_instance` 并找到显示 filesystem\_id 的那个。话虽如此，如果只有一个文件系统提供商添加到 MA，则可以使用 `filesystem_smb`。
...
更多详情[这里](/music-assistant/faq/tech-info/index.md#volume-normalization)
...
2. 使用打开动态成员选项的同步群组，或手动同步群组。当您更换房间时，将新房间加入现有群组。如何处理群组中的其他播放器取决于群组类型以及播放器是群组主播放器（同步群组）还是保存队列（手动同步）。选项是将播放器从群组中退出或静音它。有关更多信息，请阅读[群组](/music-assistant/faq/groups/index.md)

3. 使用[传输队列](/music-assistant/faq/masstransfer/index.md)操作。

## 随机播放 Spotify/播放列表/YouTube 等

您不是随机播放音乐提供商，而是在播放器的队列上启用随机播放，然后添加到队列的任何内容都会被随机播放。您可以通过在[播放器栏](/music-assistant/ui/index.md#player-bar)上选择随机播放图标在 MA 中启用队列随机播放，或者您可以选择[正在播放视图](/music-assistant/ui/index.md#now-playing-view)，然后右上角的上下文菜单，然后启用随机播放，或者您可以使用 yaml 如下：
...
另请参阅 [music\_assistant.play\_media 操作](/music-assistant/faq/massplaymedia/index.md)

## 通过脚本启动播放列表

使用上面显示的 `media_player.play_media` 操作或 `music_assistant.play_media` 操作，如[这里](/music-assistant/faq/massplaymedia/index.md)所述。

## 播放随机项目

使用 get\_library 和这样的脚本/自动化：
...
如果您使用 [TuneIn 提供商](/music-assistant/music-providers/tunein/index.md)，则您帐户中收藏的电台将出现。

如果您使用 [RadioBrowser 提供商](/music-assistant/music-providers/radio-browser/index.md)，则浏览该提供商并为所需电台选择添加到库。
...
如果您使用的是 docker，则可以使用<a href="https://www.home-assistant.io/dashboards/iframe/" target="_blank" rel="noopener noreferrer">iframe 面板</a>，或者您可以使用另一个名为 <a href="https://github.com/lovelylain/hass_ingress" target="_blank" rel="noopener noreferrer">hass\_ingress</a> 的自定义集成，它允许您向 Home Assistant 前端添加额外的 ingress 面板。
...
<a href="https://apps.nextcloud.com/apps/music" target="_blank" rel="noopener noreferrer">Nextcloud Music App</a> 支持 [Subsonic](/music-assistant/music-providers/subsonic/index.md)，因此您可以在 MA 中使用该提供商进行连接。
