...
[Music Assistant Companion App](/music-assistant/companion-app/index.md) 也可以配置为运行 squeezelite 客户端，这将允许播放到运行它的设备。
...
* 安装 https://github.com/Poeschl-HomeAssistant-Addons/mpd（这将创建一个 mpd media\_player 实体）
* 在 mpd 应用中启用 httpd\_output（允许网络流媒体）
...
Music Assistant 中可用播放器提供商的能力汇总[可在此处找到](/music-assistant/player-support/index.md)。
...
最后，如果计划分组播放器并希望使用 DSP 设置，请查看 [DSP 设置描述](/music-assistant/settings/individual-player/index.md#dsp-settings)中哪些协议在这种情况下支持 DSP。
