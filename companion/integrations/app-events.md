...
| 事件 | 原因 |
| ----- | ----- |
| `android.intent_received` | 当应用收到来自[上次更新传感器](/companion/core/sensors.md#last-update-trigger-sensor)的注册意图的广播意图时。事件数据将包含意图操作字符串和任何意图附加信息（如果有）。 |
| `android.navigation_started` | 当从 Android Auto/Automotive 中选择 `导航` 类别下的实体时。事件数据将包含选定的实体 ID。 |
| `android.zone_entered` | 进入区域。事件数据将包含所有位置数据，包括触发区域。仅限 [`full` 版本](/companion/core/android-flavors.md)用户可用。 |
| `android.zone_exited` | 离开区域。事件数据将包含所有位置数据，包括触发区域。仅限 [`full` 版本](/companion/core/android-flavors.md)用户可用。 |
