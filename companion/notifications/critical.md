...
iOS 对此类通知给予特殊优先级。关键警报始终显示在锁屏顶部，位于所有其他通知之上，即使启用了勿扰模式或 iPhone 处于静音状态也会播放声音。因为我们永远不希望您错过关键通知，所以它们也被允许绕过应用[通知速率限制](/companion/notifications/details.md)。
...
如果您之前阅读过[声音文档](/companion/notifications/sounds.md)，此语法应该大部分熟悉。注意示例扩展了 `sound` 属性以包含 `critical: 1` 标志，以及 `volume: 1.0` 将音量设置为 100%。

或者，您可以使用 [`interruption-level` 语法](/companion/notifications/basic.md#interruption-level)使通知变为关键通知。
...
:::info
以下选项仅在未使用[本地推送通知](/companion/notifications/local.md)时生效。如果您使用的是精简版本，则无需担心此问题。
:::
...
默认情况下，它们也不会覆盖勿扰设置，如果您想覆盖此设置，则需要使用[通知渠道](/companion/notifications/basic.md#notification-channels)。
