...
除了提供[位置服务](/companion/core/location.md)外，Companion 应用还向 Home Assistant 添加了多个额外的传感器。如果您不想要 `device_tracker` 实体但仍想让传感器更新，只需在[实体注册表](https://www.home-assistant.io/integrations/config/#entity-registry)中禁用该实体即可停止位置更新并保持传感器更新。
...
| `binary_sensor.focus` | 无 | 专注模式当前是否启用。需要 iOS-2021.10 或更高版本，今年晚些时候的 macOS 12 更新。如果 Home Assistant 在"允许的通知"列表中，则不会工作，详见[中断级别](/companion/notifications/basic.md#interruption-level)。 |
...
发射设置开关将启动或停止 BLE 发射。此设置以及上述大多数设置可以通过[通知命令](/companion/notifications/commands.md#ble-beacon-transmitter)更改。
...
监视设置开关将启动或停止扫描 - 此设置也可以通过[通知命令](/companion/notifications/commands.md#beacon-monitor)调整。
...
为了简化操作，您可能希望使用 [`car_ui` 参数](/companion/android-auto/index.md#notifications)在[连接手机](/companion/core/sensors.md#android-auto)时在汽车上显示通知。
:::
...
| `ip6_addresses` | 绑定到当前活动网络的 ip6\_addresses |
...
<img src="/companion-assets/android.svg" alt="Android" style="height: 1em; vertical-align: middle;" /> 此传感器的状态将反映设备当前是否启用了[高精度模式](/companion/core/location.md#high-accuracy-mode)。一旦高精度模式状态发生变化，此传感器就会更新，直到首次启用高精度模式，传感器才会出现。

## 高精度更新间隔

<img src="/companion-assets/android.svg" alt="Android" style="height: 1em; vertical-align: middle;" /> 此传感器的状态将反映[高精度模式](/companion/core/location.md#high-accuracy-mode)的设备更新间隔（秒）。一旦值发生变化（手动或通过[通知命令](/companion/notifications/commands.md#high-accuracy-mode)），此传感器就会更新。
...
对于 Android，此传感器的状态将反映最近发送更新的[意图](https://developer.android.com/reference/android/content/Intent)。此外，传感器提供设置允许用户从广播意图的其他 Android 应用接收[应用事件](/companion/integrations/app-events.md)。用户可以根据需要注册任意数量的意图，一旦收到意图，就会向 Home Assistant 发送事件。保存意图后，请务必重启应用以注册意图。
...
| Significant Location Change | 当设备的位置发生显著变化时触发，例如 500 米或更多。有关更多详细信息，请参阅[位置](/companion/core/location.md)。 |
...
| Push Notification | 通过推送通知[请求位置更新](/companion/notifications/commands.md#request-location-updates)。  |
| Background Fetch | 当应用在后台刷新传感器信息时。 |
| Siri | 通过 [Siri 快捷方式](/companion/integrations/siri-shortcuts.md) "发送位置"快捷方式触发的位置更新。 |
