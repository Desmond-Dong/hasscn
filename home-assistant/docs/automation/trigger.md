...
触发器决定了 automation 规则何时开始处理。当自动化中\_任意\_一个触发器满足条件时（触发器\_触发\_），Home Assistant 会先验证是否有[条件](/home-assistant/docs/automation/condition/index.md)，然后调用[动作](/home-assistant/docs/automation/action/index.md)。
...
所有触发器都可以分配一个可选的 `id`。如果省略 `id`，则会使用触发器在列表中的索引。你可以在[触发器条件和动作](/home-assistant/docs/scripts/conditions/index.md#trigger-condition)中引用这个 `id`。`id` 不必在每个触发器中唯一，也可以用来对相似触发器进行分组，以便后续在自动化中统一处理，例如多个不同类型的触发器都用于打开某个实体。
...
触发器可以使用两种不同类型的变量。它们都和[脚本级变量](/home-assistant/integrations/script/index.md#variables)一样工作。

第一种是在触发器触发时设置变量。这些变量可以使用模板，并能访问[`trigger` 变量](/home-assistant/docs/automation/templating.md#available-trigger-data)。

第二种是在触发器包含模板化值时，为触发器附加预先可用的变量。它们通过自动化级别的 `trigger_variables` 键定义。这些变量只能包含[受限模板](/home-assistant/docs/configuration/templating/index.md#limited-templates)。模板值变化后不会重新应用到触发器。触发器变量主要用于在触发器中支持蓝图输入。
...
当收到某个[event](/home-assistant/docs/configuration/events/index.md)时，事件触发器会触发。事件是 Home Assistant 中最基础的构建块。你既可以只匹配事件名称，也可以要求同时匹配特定的事件数据或上下文。

事件既可以由集成触发，也可以通过 API 触发。支持的事件类型没有限制。内置事件列表可见[这里](/home-assistant/docs/configuration/events/index.md)。
...
在 `event_type`、`event_data` 和 `context` 选项中也可以使用[受限模板](/home-assistant/docs/configuration/templating/index.md#limited-templates)。
...
在 `topic` 和 `payload` 选项中也可以使用[受限模板](/home-assistant/docs/configuration/templating/index.md#limited-templates)。
...
通过 `value_template` 可以完成更动态、更复杂的计算。变量 `state` 是由 `entity_id` 指定实体的[状态对象](/home-assistant/docs/configuration/state_object.md)。
...
如果这不符合你的使用场景，可以考虑让自动化先设置一个[`input_datetime`](/home-assistant/integrations/input_datetime.md) 为目标时间，再使用该 [`input_datetime`](/home-assistant/integrations/input_datetime.md) 作为自动化触发器，在设定时间执行所需动作。
...
:::tip
由于黄昏和黎明持续时间会随季节变化，建议你在需要在晨昏时段触发自动化时，使用\[太阳高度触发器]\[sun\_elevation\_trigger]，而不是给 `sunset` 或 `sunrise` 添加时间偏移。

:::
\[sun\_elevation\_trigger]: /docs/automation/trigger/#sun-elevation-trigger
...
当扫描[tag](/home-assistant/integrations/tag.md)时触发。例如，通过 Home Assistant Companion 移动应用扫描 NFC 标签。
...
模板触发器会在任意已识别实体状态变化时，对一个[模板](/home-assistant/docs/configuration/templating/index.md)进行求值。如果这次状态变化导致模板渲染结果为 `true`（非零数字，或字符串 `true`、`yes`、`on`、`enable` 中之一），而之前为 `false`（其他任何值），就会触发。
...
[input datetime](/home-assistant/integrations/input_datetime/index.md) 的实体 ID。
...
设备类为 `timestamp` 的[sensor](/home-assistant/integrations/sensor/index.md) 的实体 ID。
...
时间也支持使用[受限模板](/home-assistant/docs/configuration/templating/index.md#limited-templates)。
...
更多关于事件触发器及可供自动化使用的附加事件数据，请参阅[Persistent Notification](/home-assistant/integrations/persistent_notification/index.md) 集成文档。

## Webhook trigger

当有 Web 请求发送到 webhook 端点 `/api/webhook/<webhook_id>` 时，Webhook 触发器会触发。当你在自动化触发器中将某个值设为 `webhook_id` 时，该 webhook 端点会自动创建。`webhook_id` 可以是静态值，也可以通过[受限模板](/home-assistant/docs/configuration/templating/index.md#limited-templates)计算得到。
...
当实体进入或离开某个区域时，区域触发器会触发。实体可以是 person，也可以是 device\_tracker。要让区域自动化正常工作，你需要先设置一个支持上报 GPS 坐标的设备追踪平台，例如 [GPS Logger](/home-assistant/integrations/gpslogger/index.md)、[OwnTracks 平台](/home-assistant/integrations/owntracks/index.md) 或 [iCloud 平台](/home-assistant/integrations/icloud/index.md)。
...
当地理位置实体出现在某个区域内，或从该区域中消失时，地理位置触发器会触发。由 [Geolocation](/home-assistant/integrations/geo_location/index.md) 平台创建的实体支持 GPS 坐标上报。
...
[MQTT 设备触发器](/home-assistant/integrations/device_trigger.mqtt/index.md) 通过自动发现进行设置。
...
日历触发器会在 [Calendar](/home-assistant/integrations/calendar/index.md) 事件开始或结束时触发，
...
更多关于事件触发器及可供自动化使用的附加事件数据，请参阅 [Calendar](/home-assistant/integrations/calendar/index.md) 集成文档。

## Sentence trigger

当 [Assist](/home-assistant/voice_control/index.md) 使用默认[conversation agent](/home-assistant/integrations/conversation/index.md) 匹配到一句话时，语句触发器会触发。语句触发器可与 Home Assistant Assist 配合使用。如果使用 OpenAI 或 Google Generative AI 等外部对话代理，则不会生效，除非你在对话代理设置中启用了 “Prefer handling commands locally”。
...
* [Adding a custom sentence to trigger an automation](/home-assistant/voice_control/custom_sentences/index.md#adding-a-custom-sentence-to-trigger-an-automation)

### Sentence wildcards

在触发语句中加入一个或多个 `{lists}`，可以捕获该位置上的任意文本。捕获结果会作为 `slots` 对象[出现在触发器数据中](/home-assistant/docs/automation/templating.md#sentence)。
...
:::caution
此功能需要 Home Assistant 2024.10 或更高版本。如果你在蓝图中使用它，请将蓝图的 `min_version` 至少设置为这个版本。更多信息请参阅[蓝图 schema 文档](/home-assistant/docs/blueprint/schema/index.md#min_version)。
