...
自动化支持[模板](/home-assistant/docs/configuration/templating/index.md)的高级功能，就像脚本一样。除了脚本可用的 [Home Assistant 模板扩展](/home-assistant/docs/configuration/templating/index.md#home-assistant-template-extensions)之外，自动化还可以使用 `trigger` 和 `this` 模板变量。
...
模板变量 `this` 是一个对象，包含触发动作时该自动化的[状态](/home-assistant/docs/configuration/state_object.md)，可用于评估活动自动化配置中声明的 [`trigger_variables`](/home-assistant/docs/automation/trigger/index.md#触发器变量)。状态对象还包含上下文数据，可用于识别引发状态变化或触发执行的用户。请注意，`this` 在动作执行期间不会改变。
...
每种[触发器](/home-assistant/docs/automation/trigger/index.md#事件触发器)平台都包含该触发器特有的附加数据。
...
| `trigger.id` | [触发器的 `id`](/home-assistant/docs/automation/trigger/index.md#触发器-id)。 |
| `trigger.idx` | 触发器的索引。（第一个触发器 idx 为 `0`。） |

### 日历

以下是[日历触发器](/home-assistant/docs/automation/trigger/index.md#日历触发器)可用的属性。
...
以下是[设备触发器](/home-assistant/docs/automation/trigger/index.md#设备触发器)可用的属性。
...
每当实体状态更改或发生与配置的 event\_type 匹配的事件时，[事件](/home-assistant/docs/configuration/events/index.md)触发器就会触发。

以下是[事件触发器](/home-assistant/docs/automation/trigger/index.md#事件触发器)可用的属性。
...
以下是[地理位置触发器](/home-assistant/docs/automation/trigger/index.md#地理位置触发器)可用的属性。
...
对于自动化，推荐使用 Home Assistant 触发器，而不是 [homeassistant\_start 或 homeassistant\_stop 事件](/home-assistant/docs/configuration/events/index.md#homeassistant_start-homeassistant_started)。

以下是 [Home Assistant 触发器](/home-assistant/docs/automation/trigger/index.md#home-assistant-触发器)可用的属性。
...
以下是 [MQTT 触发器](/home-assistant/docs/automation/trigger/index.md#mqtt-触发器)可用的属性。
...
以下是[数值状态触发器](/home-assistant/docs/automation/trigger/index.md#数值状态触发器)可用的属性。
...
以下是[语句触发器](/home-assistant/docs/automation/trigger/index.md#语句触发器)可用的属性。
...
| `trigger.details`  | 按名称匹配的槽位详细信息的对象，例如[通配符](/home-assistant/docs/automation/trigger/index.md#sentence-wildcards)。每个详细信息包含：<ul><li>`name` - 槽位名称</li><li>`text` - 匹配的文本</li><li>`value` - 输出值（参见[列表](/home-assistant/docs/voice/intent-recognition/template-sentence-syntax/#lists)）</li></ul> |
| `trigger.device_id` | 捕获命令的设备 ID（如果有）。 |
| `trigger.satellite_id` | 捕获命令的卫星实体 ID（如果有）。 |

### 状态

以下是[状态触发器](/home-assistant/docs/automation/trigger/index.md#状态触发器)可用的属性。
...
以下是[太阳触发器](/home-assistant/docs/automation/trigger/index.md#太阳触发器)可用的属性。
...
以下是[标签触发器](/home-assistant/docs/automation/trigger/index.md#tag-trigger)可用的属性。
...
以下是[模板触发器](/home-assistant/docs/automation/trigger/index.md#模板触发器)可用的属性。
...
以下是[时间触发器](/home-assistant/docs/automation/trigger/index.md#time-trigger)可用的属性。
...
以下是[时间模式触发器](/home-assistant/docs/automation/trigger/index.md#时间模式触发器)可用的属性。
...
以下是[持久通知触发器](/home-assistant/docs/automation/trigger/index.md#持久通知触发器)可用的属性。
...
以下是 [Webhook 触发器](/home-assistant/docs/automation/trigger/index.md#webhook-触发器)可用的属性。
...
以下是[区域触发器](/home-assistant/docs/automation/trigger/index.md#区域触发器)可用的属性。
