...
本教程使用 [Random 传感器](/home-assistant/integrations/random.md#sensor)，因为它可以生成数据（默认情况下，值在 0 到 20 之间）。即使您还没有连接任何实际传感器，也可以跟着本示例操作。您也可以使用任何其他输出数值的传感器。
...
4. 输入触发器条件：
   ...
   * 定义传感器：在 **实体** 下，输入 "sensor.random\_sensor"。
...
6. 我们要创建一个 [持久通知](/home-assistant/integrations/persistent_notification/index.md)。
...
8. 选择 **保存**，为您的自动化起一个有意义的名称，然后再次 **保存**。
   ...
   * 要了解更多关于自动化的内容，请阅读 [自动化 Home Assistant](/home-assistant/getting-started/automation/index.md) 文档。
