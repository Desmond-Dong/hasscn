...
当脚本在自动化中执行时，可以使用 `trigger` 变量。请参阅[可用的触发器数据](/home-assistant/docs/automation/templating/index.md#可用的触发器数据)。
...
当模板中引用的实体 ID 状态发生变化时，模板会重新计算。如果您在模板中使用了 `now()` 这类非确定性函数，它不会持续重新计算，而只会在被引用的实体 ID 发生变化时重新计算。如果您需要定期重新计算模板，可以引用 [Time and Date](/home-assistant/integrations/time_date/index.md) 集成中的某个传感器，该传感器会按分钟或按天更新。
...
此动作可以使用与自动化 `trigger` 部分相同的触发器。请参阅[自动化触发器](/home-assistant/docs/automation/trigger/index.md)。当任一触发器触发时，脚本都会继续执行。此前定义的[触发器变量](/home-assistant/docs/automation/trigger/index.md#触发器变量)、[变量](#变量)以及[脚本变量]都会传递给该触发器。
...
| `wait.trigger`   | 仅在 `wait_for_trigger` 之后存在。包含哪个触发器被触发的信息。（请参阅[可用的触发器数据](/home-assistant/docs/automation/templating/index.md#可用的触发器数据)。）如果超时前没有触发器发生，则为 `none` |
...
下面的示例展示了如何使用[自动化事件触发器](/home-assistant/docs/automation/trigger/index.md#事件触发器)捕获自定义事件 `event_light_state_changed`，并获取作为事件触发器数据传入的 `entity_id`。更多详情请参阅[可用的触发器数据](/home-assistant/docs/automation/templating/index.md#可用的触发器数据)。
...
这种形式接受一组条件（可用选项请参阅[条件页面]），并会在每次运行序列 *之前* 进行评估。
...
This form accepts a list of 条件 that are evaluated *after* each time the sequence
is run. Therefore the sequence will always run at least once. The sequence will be run
*until* the 条件(s) evaluate to true.
...
This  allows you to conditionally (`if`), based on or more [条件](/home-assistant/docs/scripts/conditions/index.md) (which are `and` combined),
