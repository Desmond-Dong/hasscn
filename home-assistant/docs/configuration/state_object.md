...
状态一次只保存一个值。但是，实体可以在状态对象中存储相关的实体状态属性。例如，灯的状态是 *on*，相关属性可能是其当前亮度和颜色值。[状态变更事件](/home-assistant/docs/configuration/events/index.md#events-and-state-changes)可以用作触发器。当前状态可用于[条件](/home-assistant/docs/automation/condition/index.md)。下面的示例显示了三个具有不同实体状态属性的灯。
