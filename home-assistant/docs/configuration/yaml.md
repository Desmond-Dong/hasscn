...
Home Assistant 使用 [YAML](https://yaml.org/) 语法进行配置。虽然大多数集成可以通过用户界面进行配置，但有些集成需要您编辑 [`configuration.yaml`](/home-assistant/docs/configuration/index.md) 文件来指定其设置。
...
以下 YAML 示例假设您想要使用 [pushbullet 平台](/home-assistant/integrations/pushbullet.md) 设置 [notify 集成](/home-assistant/integrations/notify.md)。
...
下一个示例展示了一个 [input\_select](/home-assistant/integrations/input_select.md) 集成，它使用块集合来表示选项值。
其他属性（如 `name:`）使用映射指定。请注意，第二行只有 `threat:`，同一行上没有值。在这里，`threat` 是 input\_select 的名称。它的值是嵌套在其下方的所有内容。
...
有关此功能的更多信息，请参阅[拆分配置](/home-assistant/docs/configuration/splitting_configuration/index.md)。
...
* 如果您直接在 Home Assistant 中编辑文件，请参阅：[验证配置](/home-assistant/docs/configuration/index.md#validating-the-configuration)
