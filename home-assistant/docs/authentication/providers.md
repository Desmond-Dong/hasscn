...
当您登录时，*认证提供商* 会检查您的凭据以确保您是已授权的用户。
...
这是默认的认证提供商。第一个创建的用户被指定为\_所有者\_，可以创建其他用户。
...
:::note
如果您使用此认证提供商，[多因素认证模块](/home-assistant/docs/authentication/multi-factor-auth/index.md) 将不会参与登录过程。
:::

:::important
您不能信任您在任何 [trusted\_proxies](/home-assistant/integrations/http/index.md#reverse-proxies) 中使用的网络。`trusted_networks` 认证将失败，并显示消息：您的计算机不被允许
:::
