...
您可以在用户账户的[个人资料页面](/home-assistant/docs/authentication/index.md#your-account-profile)中开启或关闭 MFA。
...
:::note
TOTP 是\_基于时间\_的，因此它依赖于您的 Home Assistant 时钟准确。如果验证持续失败，请确保 Home Assistant 上的时钟正确。
:::

### 通知多因素认证模块

通知 MFA 模块使用 [notify 集成](/home-assistant/integrations/notify/index.md)向您发送 [基于 HMAC 的一次性密码](https://en.wikipedia.org/wiki/HMAC-based_One-time_Password_algorithm)。该密码通常会发送到您的手机，但也可以发送到 `notify` 动作支持的任何目标。您使用此密码登录。
