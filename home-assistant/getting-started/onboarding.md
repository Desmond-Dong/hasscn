...
在设备上[安装](/home-assistant/installation/index.md) Home Assistant 后，需要完成 5 个步骤来设置 Home Assistant。

1. 在浏览器地址栏中输入以下 URL：<http://homeassistant.local:8123/>。

   * **结果**：您现在看到 **正在准备 Home Assistant** 页面。根据您的硬件和网络连接，准备可能需要一些时间。
     * Home Assistant 正在下载最新版本的 **Home Assistant Core**（约 700 MB）。
   * 如果此步骤遇到问题，请参阅[安装故障排除](/home-assistant/installation/troubleshooting/index.md)。
   * 准备完成后，将显示欢迎屏幕。
...
2. 您可以创建新安装或从备份恢复现有安装：
   ...
   * **选项 2：从备份恢复**：如果您想从以前的安装备份恢复，请按照[从备份恢复](/home-assistant/common-tasks/general/index.md#restoring-a-backup)的步骤操作。
...
3. 输入您的家庭位置。
   ...
   * 家庭位置用于配置时区、单位系统和货币。
   * 它还用于创建家庭[区域](/home-assistant/integrations/zone/index.md)，默认半径为 100 米。
   * 您可以稍后在设置中更改此信息。
...
5. 您的 Home Assistant 已经启动运行。
   * 按 **完成**，您现在可以看到默认[仪表盘](/home-assistant/dashboards/index.md)。

:::info [概念与术语](/home-assistant/getting-started/concepts-terminology/index.md)
:::
