...
* 添加 测试 为 check\_config 辅助工具 ([@emontnemery] - [#102898])
* 改进 dlna\_dmr 测试 ([@emontnemery] - [#102905])
* 修复 内部 回调 装饰器 使用 偏函数 ([@bdraco] - [#102873])
* 更新 prettier 到 3.0.3 ([@scop] - [#102929])
* 小幅清理 mobile\_app加密 ([@bdraco] - [#102883])
...
* 允许 setting hvac mode through set\_temperature 温控 method 在 Gree 集成 ([@MislavMandaric] - [#101196])
...
* 添加 严格类型标注 为 input\_text ([@jbouwh] - [#103095])
...
* 将 基础实体 的 system\_bridge 移动到独立模块 ([@jpbede] - [#103167])
...
* Small speed up 到 async\_listen ([@bdraco] - [#103307])
...
* 设置 device entry type 为 dwd\_weather\_warnings ([@jpbede] - [#103370])
* 移除 deprecated /config/server\_control redirect ([@jpbede] - [#103331])
...
* 将 vallox\_WebSocket\_API 到 4.0.2 ([@slovdahl] - [#103339])
* 修复 CalDAV supported components check when configured 从 UI ([@allenporter] - [#103411])
* Differentiate between warnings 和 errors 在 check\_config 辅助工具 ([@emontnemery] - [#102902])
...
* 允许 setting HVAC mode through set\_temperature 服务 在 Airzone 集成 ([@Noltari] - [#103185])
* 允许 setting HVAC mode through set\_temperature 服务 在 Airzone Cloud 集成 ([@Noltari] - [#103184])
* 重构 测试 为 Islamic Prayer Times ([@engrbm87] - [#103439])
* Store Islamic Prayer Times 协调器 在 'entry\_id' key ([@engrbm87] - [#103405])
* 将 evohome-async 到 0.4.4 ([@zxdavb] - [#103084])
* 添加 Check date 服务 为 Workday ([@gjohansson-ST] - [#97280])
* Validate entity category 为 binary\_传感器 ([@jbouwh] - [#103464])
* On Airzone cloud unload logout ([@Noltari] - [#103487])
* 将 pycfdns 从 2.0.1 到 3.0.0 ([@ludeeus] - [#103426])
* Split out 协调器 和 add 测试 为 nibe\_heatpump ([@elupus] - [#103452])
...
* 将 reolink\_aio 到 0.7.15 ([@starkillerOG] - [#103548])
...
* 添加 hvac\_action support 到 melcloud ([@ffourcot] - [#103372])
* 小幅清理 process\_success\_login ([@bdraco] - [#103282])
...
* Retrieve manufacturer 和 configuration\_url 从 client 在 Overkiz 集成 ([@iMicknl] - [#103585])
...
* 修复 metoffice 测试\_forecast\_subscription raises key error ([@jbouwh] - [#103598])
...
* 支持 continue\_on\_error 为 command execution 在 Overkiz ([@iMicknl] - [#103591])
...
* Humanize core config errors 在 check\_config 辅助工具 ([@emontnemery] - [#103635])
...
* 修复 entity category 为 binary\_传感器 fails setup ([@jbouwh] - [#103511])
...
* Attach correct platform config 在 check\_config warnings 和 errors ([@emontnemery] - [#103633])
...
* 添加 脚本\_mode parameter 到 custom intent 脚本s ([@tetele] - [#102203])
...
* Tweak loader.resolve\_dependencies ([@emontnemery] - [#103851])
* 实现 update 协调器 在 Proximity ([@mib1185] - [#103443])
* 移除 useless code 从 entity 辅助工具 测试 ([@emontnemery] - [#103854])
* Tweak config.\_recursive\_merge ([@emontnemery] - [#103850])
...
* 修复 invalid oauth2\_unauthorized translation ref ([@jbouwh] - [#103873])
...
* 添加 support 到 fints 为 configuring unsupported account\_types ([@mricharz] - [#83537])
...
* 修复 类型标注 为 entity\_platform.async\_register\_entity\_服务 ([@KapJI] - [#103777])
...
* 移除 unneeded self.async\_write\_ha\_state() 在 Blink ([@mkmer] - [#103932])
* 将 ring\_doorbell 到 0.8.0 和 handle new exceptions ([@sdb9696] - [#103904])
...
* 改进 docstring 的 config.stringify\_invalid ([@emontnemery] - [#103965])
* 添加 测试 为 component configuration 使用 multiple errors ([@emontnemery] - [#103964])
* 添加 broadlink 温控 ([@eifinger] - [#91183])
* Removed 代码所有者 为 growatt\_server ([@muppet3000] - [#103970])
* 添加 reply\_to\_消息\_id 到 all telegram\_bot 消息 types ([@TheKalpit] - [#103566])
...
* 添加 todo.get\_items 服务 ([@allenporter] - [#103285])
...
* 添加 zwave\_js.set\_lock\_configuration 服务 ([@raman325] - [#103595])
* 重构 config.async\_log\_exception ([@emontnemery] - [#104034])
...
* Attach relevant config 到 check\_config errors ([@emontnemery] - [#104048])
* 添加 reauth support 到 Schlage ([@dknowles2] - [#103351])
* 移除 force\_update 从 all DSMR entities ([@gigatexel] - [#104037])
...
* 移除 mock\_entity\_platform 测试 辅助工具 ([@emontnemery] - [#104073])
...
* 添加 catsmanac 到 enphase\_envoy 代码所有者s ([@catsmanac] - [#104086])
...
* 避免 duplicate calls 到 color\_supported 和 color\_temp\_supported 在 emulated\_hue ([@bdraco] - [#104096])
...
* 修复 colors 在 check\_config 脚本 ([@emontnemery] - [#104069])
...
* 重构 async\_update 在 Honeywell ([@mkmer] - [#103069])
* New API endpoint 为 Trafikverket Weather ([@gjohansson-ST] - [#104165])
* Small speed up 到 constructing Bluetooth 服务\_uuids ([@bdraco] - [#104193])
* Deprecate calendar.list\_events ([@eifinger] - [#102481])
...
* Don't mutate config 在 check\_config 辅助工具 ([@emontnemery] - [#104241])
* 将 some check\_config 测试 cases ([@emontnemery] - [#104244])
...
* 使用 send\_json\_auto\_id 在 todo 测试 ([@emontnemery] - [#104245])
...
* 设置 unique\_id by 基础实体 在 ViCare 集成 ([@CFenner] - [#104277])
...
* Plugwise: limit \_attr\_max\_temp 到 35.0 为 thermostats that report a max 的 100. ([@bouwew] - [#104324])
* 移除 MTrab 从 Repetier CodeOwners ([@MTrab] - [#104356])
* 修复 idasen\_desk 协调器 类型标注 ([@cdce8p] - [#104361])
...
* Migrate Aurora\_ABB\_Powerone 到 DataUpdateCoordinator ([@davet2001] - [#72363])
...
* 添加 support 到 fully\_kiosk 为 hybrid local push/pull switches 使用 MQTT ([@mheath] - [#89010])
* Blebox update IP address if 已经 configured via zeroconf ([@riokuu] - [#90511])
* 添加 select entity 为 zwave\_js Door Lock CC ([@raman325] - [#104292])
...
* 支持 为 group into command\_line auth provider ([@Hejki] - [#92906])
...
* 将 ring\_doorbell 到 0.8.2 使用 listen extra ([@sdb9696] - [#104033])
...
* 添加 alert 到 zwave\_js 设备信息 page 为 custom device config ([@raman325] - [#104115])
...
* 将 ring\_doorbell 到 0.8.3 ([@sdb9696] - [#104611])
...
* 修复 human readable huawei\_lte 传感器 names ([@scop] - [#104672])
...
* 添加 PECO smart meter binary\_传感器 ([@IceBotYT] - [#71034])
...
* Make huawei\_lte operator search 和 preferred network modes translatable ([@scop] - [#104673])
* Send localization info 在 WebSocket\_API 脚本 errors ([@jbouwh] - [#104638])
...
* Assign specific error code 为 HomeAssistantError 在 WebSocket\_API connection exceptions ([@jbouwh] - [#104700])
* 将 todo due\_date\_time parameter 到 due\_datetime ([@MartinHjelmare] - [#104698])
...
* 使用 ServiceValidationError 为 invalid fan preset\_mode 和 move check 到 fan entity component ([@jbouwh] - [#104560])
...
* 移除 redundant WebSocket\_API 异常 handler ([@jbouwh] - [#104727])
* 修复 TTS streaming 为 VoIP ([@synesthesiam] - [#104620])
* 将 bimmer-connected 到 0.14.5 ([@rikroe] - [#104715])
* 移除 aiohttp enable\_compression 辅助工具 ([@bdraco] - [#104174])
...
* 修复 wirelesstag unique\_id 到 use uuid 而不是 tag\_id ([@sergeymaysak] - [#104394])
* 修复 Philips TV none recordings\_list ([@Floyer007] - [#104665])
...
* 添加 NodeStrClass.**voluptuous\_compile** ([@emontnemery] - [#104808])
...
* 使用 deprecated\_class decorator 在 deprecated YAML loader classes ([@emontnemery] - [#104835])
...
* 修复 get\_events name 在 calendar strings ([@gjohansson-ST] - [#104902])
...
* Don't use deprecated\_class decorator 在 deprecated YAML classes ([@emontnemery] - [#105063])
* 移除 device 从 known\_devices upon import 在 ping device tracker ([@jpbede] - [#105009])
* 更新 frontend 到 20231205.0 ([@bramkragten] - [#105081])
* 更新 apple\_weatherkit 到 1.1.1 ([@tjhorner] - [#105079])
...
* Initialize energy\_state without price ([@Excentyl] - [#97031])
* 将 yolink-API 到 0.3.4 ([@matrixd2] - [#105124])
* 将 apple\_weatherkit 到 1.1.2 ([@tjhorner] - [#105140])
...
* 将 reolink\_aio 到 0.8.2 ([@starkillerOG] - [#105157])
...
* 修复 check\_date 服务 在 workday ([@gjohansson-ST] - [#105241])
...
* Always set \_attr\_current\_option 在 Nobø Hub select entities ([@oyvindwe] - [#105289])
...
* 修复 CI 测试\_invalid\_rrule\_fix 测试 by freezing time ([@jbouwh] - [#105294])
...
* 添加 missing rest\_command reload 服务 到 服务s.yaml ([@jpbede] - [#105714])
...
* 将 reolink\_aio 到 0.8.3 ([@starkillerOG] - [#105489])
* 将 reolink\_aio 到 0.8.4 ([@starkillerOG] - [#105946])
...
* 修复 Shelly consumption\_types ([@emontnemery] - [#106273])
