...
借助 [@robbiet480] 和 [@pvizeli] 新增的 [IP Webcam](/home-assistant/integrations/android_ip_webcam/index.md) 支持，你现在可以把任何一台 Android 手机改造成多功能 IP 摄像头。你可以做到下面这些事情：
...
* Cleanup run\_callback\_threadsafe ([@pvizeli])
...
* improve history\_stats accuracy ([@bokub])
* Updated pyitachip2ir ([@alanfischer])
* Influx fix ([@open-homeautomation])
* Fix 切换 and media\_play\_pause post async ([@armills])
...
* Cleanup component track\_point\_in\_utc\_time usage ([@pvizeli])
...
* Zwave optimize value\_added ([@andrey-git])
* 更新 Vagrant provision.sh ([@shaftoe])
* 更新 Adafruit\_Python\_DHT to support new raspberry kernel ([@masarliev])
...
* 模板 binary\_sensor change flow / add 恢复 ([@pvizeli])
...
* Bugfix new async\_add\_devices function ([@pvizeli])
* 恢复 for input\_slider ([@pvizeli])
* Added IPv4 data collector ([@open-homeautomation])
* Return None instead of raising ValueException from as\_timestamp 模板 function. ([@jjmontesl])
...
* Fix wake\_on\_lan for German 版本 of Windows 10 (#6397) ([@siebert])
...
* 恢复 flow on device\_tracker platform ([@pvizeli])
...
* Tweak recorder/restore\_state ([@balloob])
* Fix unnecessary 警告 for ip bans.yaml ([@balloob])
* Better restore\_state 警告 ([@balloob])
...
* Rename \_scheduled\_update to \_update\_scheduled ([@andrey-git])
* Revert "Use dynamic port allocation for tests" ([@armills])
* Tado device\_tracker exception when mobile 设备 has geofencing enabled but location is currently unknown. ([@jmvermeulen])
...
* Fix wake\_on\_lan ping for Linux. ([@siebert])
...
* Bugfix rpi\_rf cleanup ([@pvizeli])
