...
作为 Connected Home over IP 公告的一部分，Apple 发布了其 Accessory Development Kit（ADK）的[开源版本](https://github.com/apple/HomeKitADK)。ADK 让设备可以被 Home Assistant（当然也包括 iOS 设备）这类 HomeKit 控制器控制。该规范此前已经开放，Home Assistant 也已通过 [HomeKit 集成](/home-assistant/integrations/homekit/index.md)实现支持。现在 ADK 开源依然很有价值，因为它提供了规范应如何落地的参考实现。
