...
你的 `target_dir` 目录会开始不断出现摄像头生成的图像。`motion` 会创建一个名为 `lastsnap.jpg` 的符号链接，它始终指向最新快照。接下来我们会把 [Local File 摄像头平台](/home-assistant/integrations/local_file.md) 设置为读取这个文件。
...
在这种场景下，你需要在 `configuration.yaml` 里配置 [Generic MJPEG IP 摄像头](/home-assistant/integrations/mjpeg.md)。
