...
*请注意，这是首次发布。有些功能还会缺失，也可能还有一些地方没有完全正常工作。*
...
* New component 'insteon\_plm' and related platforms ([@nugget])
...
* 传感器 - Amcrest: Avoid traceback for Amcrest 摄像头/firmware that does not have the software\_information API call ([@tchellomello])
...
* Z-Wave: Add device\_class support to 遮盖 component ([@armills])
* Config: Add check\_config API ([@balloob])
* Media player: Add media\_image to 媒体播放器 component ([@postlund])
* 传感器- Vasttrafik.py: vasttrafik: 更新 token on read 错误 ([@persandstrom])
* Z-Wave.py: force\_update zwave 传感器 ([@andrey-git])
* 核心: Two stage shutdown ([@pvizeli])
* Z-Wave: Rename customize to device\_config ([@balloob])
...
* HDMI-CEC: HDMI\_CEC customization \[Breaking change] ([@balloob], [@andrey-git])
...
* Media Player -Sonos: Bugfix sonos favorite\_source after lost connection ([@pvizeli])
...
* 灯光 - Hue: Add effect\_list to hue 灯光 ([@armills])
...
* 灯光 - Flux\_LED: 更新 FLUX\_LED by adding Effects ([@dramamoose])
* 报警 control 面板 - Simplisafe: SimpliSafe updates ([@w1ll1am23])
* 遮盖 Add supported\_features to 遮盖 component ([@armills])
...
* 恢复: Restore\_state 助手 to 恢复 实体 状态 from the DB on startup ([@kellerza])
* 传感器 - Darksky: Add 'entity\_picture' to Darksky component ([@aronsky])
...
* tests/integrations/device\_tracker/test\_init.py: 恢复 for device\_tracker ([@kellerza])
...
* hdmi\_cec config now requires users to set the types in the hdmi\_cec config instead of using `customize`:
