...
2. 启动附加组件（无需“启动时启动”，您只需运行一次）。

3. 导航至附加组件的“日志”并按照其中给出的说明进行操作。

4. 完成后，重新启动 Home Assistant 并导航至[设备页面](https://my.home-assistant.io/redirect/devices/)。添加集成 HACS，然后按照说明将其与您的 GitHub 帐户关联。
