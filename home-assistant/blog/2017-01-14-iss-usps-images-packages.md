...
[InfluxDB][influx] 组件此前在很多不同的使用场景下都会引发问题。[@titilambert] 改进了我们的 InfluxDB 导出功能。你可能需要运行这个[迁移脚本](/home-assistant/integrations/influxdb/index.md#migration-脚本)，来更新你的 InfluxDB 数据库。
...
* Fix load\_yaml default value ([@balloob])
* Fix discovery of flux\_led ([@Danielhiversen])
* Fix Python Nest dependency re-安装 ([@Danielhiversen])
* Make USPS to use absolute path to save cookie ([@tchellomello])
* Fix UPC\_connect cookies ([@pvizeli])
...
* [InfluxDB][influx] component has a new [schema](/home-assistant/integrations/influxdb/index.md#data-migration) to store values in the InfluxDB database. You may require to run the [`influxdb_migrator`](/home-assistant/integrations/influxdb/index.md#migration-脚本) 脚本.
  You have to note:
  ...
  * All string fields corresponding to 状态 属性 will be renamed as FIELDNAME\_str, where FIELDNAME is the 状态 属性, to avoid type conflicts.
