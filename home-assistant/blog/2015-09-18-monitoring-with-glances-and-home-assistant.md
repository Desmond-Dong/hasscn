...
[Glances 传感器](/home-assistant/integrations/glances.md) 就是通过这个 API 获取所需数据。
...
然后访问 http://IP\_ADRRESS:61208/，你应该能看到 Glances 的 Web 界面。
...
还可以再检查一次：访问位于 http://IP\_ADRRESS:61208/api/2/mem/used 的 API，并确认它会返回包含内存使用详情的 JSON 响应。如果没问题，就可以继续了。
...
把 [glances 传感器](/home-assistant/integrations/glances.md) 配置项添加到 `configuration.yaml`，然后重启 Home Assistant。
