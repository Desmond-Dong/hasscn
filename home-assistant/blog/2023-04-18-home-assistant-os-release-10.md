...
数据盘功能允许你通过外接磁盘扩展存储。启用数据盘后，所有高频读写数据都会移动到该存储上，只有 Home Assistant OS 本体仍保留在原始存储（例如 SD 卡或 eMMC）上。Home Assistant OS 是只读操作系统 — 只有在更新操作系统时才会写入 OS 分区。这能确保原有存储的写入磨损降到最低。
...
*部分产品链接为联盟链接，当你通过这些链接购买时，我们会获得少量佣金。*
