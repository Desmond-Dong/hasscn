...
1. 检查您的配置目录是否包含大型数据库文件：
   ...
   * 默认情况下，数据保留 10 天。如果您将其修改为更长时间，请检查 [`recorder`](/home-assistant/integrations/recorder/index.md) 集成页面，了解如何将数据库数据保持在不会引起问题的大小的选项。
   * 记录保留天数、清除间隔和包含/排除选项。
...
3. 如果您想将备份存储在网络存储上而不仅仅是本地系统上，请按照[添加新的网络存储](/home-assistant/common-tasks/os/index.md#添加新的网络存储)中的步骤操作，并选择 **备份** 选项。
...
3. 下载应急工具包并将其存储在安全的地方。
   * 您需要它来恢复加密备份。
   * 要了解有关备份加密的更多信息，请参阅[备份应急工具包](/home-assistant/more-info/backup-emergency-kit/index.md)的文档。
4. 定义备份时间表。
...
:::note
您可以在[这里](/home-assistant/integrations/index.md#backup)找到提供备份位置的集成概述。
:::

#### 关于 Home Assistant Cloud 上的备份存储

如果您有 Home Assistant Cloud，您可以在 Home Assistant Cloud 上存储最大 5 GB 的备份。此云存储空间对所有现有和新的 Home Assistant Cloud 订阅者免费提供。它存储一个备份文件：最后保存到 Home Assistant Cloud 的备份。这些备份始终是加密的。要恢复加密备份，您需要存储在[备份应急工具包](/home-assistant/more-info/backup-emergency-kit/index.md)中的加密密钥。
...
2. 在 **位置** 下，使用切换开关启用您想要使用的所有备份位置。
   ...
   * 如果您想备份到您的 NAS（如 [Synology](/home-assistant/integrations/synology_dsm/index.md#backup-location)）或云提供商（如 [Google Drive](/home-assistant/integrations/google_drive/index.md) 或 [Microsoft OneDrive](/home-assistant/integrations/onedrive/index.md)），请查看它们的集成文档，了解设置 Home Assistant 备份的具体说明。
   * 如果您没有看到网络存储，说明您还没有添加。请按照[添加新的网络存储](/home-assistant/common-tasks/os/index.md#添加新的网络存储)中的步骤操作，并选择 **备份** 选项。
...
如果 UI 中提供的备份自动化设置不符合您的用例，您可以使用 [backup.create\_automatic](/home-assistant/integrations/backup/index.md#action-backupcreate_automatic) 操作手动配置您自己的备份自动化。
...
6. 下载[备份应急工具包](/home-assistant/more-info/backup-emergency-kit/index.md)并将其存储在安全的地方。确保记录它所属的备份名称。
7. 要开始备份过程，选择 **创建备份**。
...
1. 如果您还没有这样做，请使用其中列出的方法之一[配置对 Home Assistant 文件的访问](/home-assistant/common-tasks/os/index.md#配置文件访问权限)。
   * 例如，[安装和使用 Samba 插件](/home-assistant/common-tasks/os/index.md#安装和使用-samba-插件)。
...
* 此过程假设您已在目标设备上完成了[安装](/home-assistant/installation/index.md)过程，现在正在查看[初始设置](/home-assistant/getting-started/onboarding/index.md)的欢迎屏幕。
* 您制作备份的设备的登录凭据。
* 包含解密备份所需密钥的[备份应急工具包](/home-assistant/more-info/backup-emergency-kit/index.md)。
...
4. 输入存储在[备份应急工具包](/home-assistant/more-info/backup-emergency-kit/index.md)中的加密密钥。
5. 要开始过程，选择 **恢复备份**。
   * 恢复可能需要一段时间，取决于数据量。
   * 不要刷新页面。只需等待。
     * 如果您刷新页面，您将看到 "未找到" 消息。这是因为系统正在关闭、擦除并从备份重新安装。在此期间，它将无法访问。
   * 如果您之前的安装直接为 [`http` 集成](/home-assistant/integrations/http/index.md)启用了证书，当恢复完成时，它将不再响应 `http://` 请求。在这种情况下，使用 `https://`（添加 `s`）代替。
6. 在登录屏幕上，输入您制作备份的系统的凭据。
   * 登录密码和用户名必须与您制作备份时使用的密码和用户名匹配。
   * 您的仪表盘应显示创建备份时的所有元素。
   * 如果某些设备显示为不可用，您可能需要唤醒电池供电的设备。
7. 如果您在以前的系统上连接了[网络存储](/home-assistant/common-tasks/os/index.md#网络存储)，您可能需要重新连接它们。
8. 如果您有 Zigbee 设备，并且您迁移到了具有内置 Zigbee 无线电的新设备：
   * 因为这现在是不同的 Zigbee 无线电，您需要[迁移 Zigbee](/home-assistant/integrations/zha/index.md#migrating-to-a-new-zigbee-coordinator-adapter-inside-zha)。
...
2. [启用多选](/home-assistant/docs/organizing/tables.md) 并选择所有要启用或禁用的实体。
...
* [自定义实体](/home-assistant/docs/configuration/customizing-devices/index.md)
* [分组您的资产](/home-assistant/docs/organizing/index.md)
* [使用表格](/home-assistant/docs/organizing/tables.md)
...
1. 并非所有集成都有可配置的轮询间隔。另一方面，homeassistant.update\_entity 服务适用于大多数集成；不需要代码更改。
...
3. 要定义您的自定义轮询间隔，创建一个自动化。
   ...
   * 定义您喜欢的任何触发器和条件。
   * 选择 **添加操作**，然后选择 **其他操作**。
   * 选择 **执行操作**，从列表中选择 [`homeassistant.update_entity` 操作](/home-assistant/integrations/homeassistant/index.md#action-homeassistantupdate_entity)。
