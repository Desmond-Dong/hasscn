...
collection\_key:
required: false
description: "如果使用 `period: energy_date_selection`，您可以设置一个自定义键，以匹配 `energy-date-selection` 卡片的可选键。这通常不是必需的，但如果同一视图中使用了多个日期选择卡片，会很有帮助。请参阅[能量文档](/home-assistant/dashboards/energy/index.md#using-multiple-collections)。"
type: string
