...
对于状态类为 measurement、total 或 total\_increasing 的传感器，系统会每 5 分钟收集一次统计数据，并每小时再汇总一次。5 分钟级统计数据会按[记录器配置](/home-assistant/integrations/recorder/index.md#purge_keep_days)中设置的时长保留，而小时级统计数据会无限期保留。它会为特定小时保留传感器数值的 <abbr title="Minimum">`min`</abbr>、<abbr title="Maximum">`max`</abbr> 和 <abbr title="Average">`mean`</abbr>，或者为计量实体保留 <abbr title="Total">`sum`</abbr>。

如果您的传感器无法使用统计功能，请查看[这篇说明](/home-assistant/more-info/statistics/index.md)。
