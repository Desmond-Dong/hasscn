...
如果实体的 state\_class 为 measurement、total 或 total\_increasing，Home Assistant 会为其保存长期统计数据。对于长期统计，会从传感器历史中存储每小时聚合数据。长期统计永远不会被清除。
...
或者使用更长的时间范围，并在一个图表中显示多个实体（只要它们共享相同的 unit\_of\_measurement）：
