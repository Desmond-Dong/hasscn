...
要使用此集成，运行 Home Assistant 的设备上必须已正确设置并可用 [Bluetooth](/home-assistant/integrations/bluetooth.md)。此集成不要求必须配备 [SwitchBot Hub](https://switch-bot.com/pages/switchbot-hub-mini)。
...
如需查看通过 SwitchBot Matter 认证的设备列表，请访问 [SwitchBot Matter](/home-assistant/integrations/switchbot_matter/index.md) 页面。
...
* `last_run_success`：若上一次发送给 SwitchBot 的操作成功，则返回 `true`。当蓝牙连接偶发不稳定时，这个属性有助于排查错误。若为 `false`，请查看 [Home Assistant 日志](/home-assistant/integrations/logger/index.md#viewing-logs) 了解具体错误信息。
...
某些集成可能会将你的 SwitchBot Blind Tilt 提供给其他动作使用，而那些动作默认认为 100% 为打开、0% 为完全关闭。你可以使用 [Cover Template](/home-assistant/integrations/template/index.md#cover) 创建一个代理实体，使其在 100% 时表示打开、0% 时表示关闭。这个模板实体仅支持单方向关闭。
...
请将设备移近一些，或更换速度更快的蓝牙适配器。更多信息请参阅[改善连接时间](/home-assistant/integrations/bluetooth/index.md#improving-connection-times)。
...
启用且正确运行 [Bluetooth](/home-assistant/integrations/bluetooth.md) 集成后，SwitchBot 集成会自动发现设备。
