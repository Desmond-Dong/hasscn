...
要使用 Smappee 云集成，您需要个人 `client_id` 和 `client_secret`，并将它们添加到 `configuration.yaml` 文件中。对于个人使用，API 访问是免费的，您可以通过联系 <support@smappee.com> 获取凭据。对于商业用途，API 访问需要定期付费，您可以通过联系 <info@smappee.com> 获取凭据。
