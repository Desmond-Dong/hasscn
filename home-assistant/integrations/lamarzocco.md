...
如果您的 Home Assistant 主机支持 [DHCP 发现](https://www.home-assistant.io/integrations/dhcp/)，您的咖啡机会被自动发现。否则，只要咖啡机处于 Home Assistant 主机的蓝牙范围内，且 [Bluetooth](/home-assistant/integrations/bluetooth.md) 集成已完整加载，也同样可以被发现。
...
<iconify-icon inline title="La Marzocco Cloud" icon="material-symbols:cloud-outline"></iconify-icon> = La Marzocco Cloud <iconify-icon inline title="Bluetooth" icon="material-symbols:bluetooth"></iconify-icon> = Bluetooth
