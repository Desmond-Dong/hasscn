...
要使用 MQTT，请在安装中选择实体 [add a MQTT device as a subentry](/home-assistant/integrations/mqtt/index.md#configuration)，或将以下内容添加到 `configuration.yaml` 文件中。
...
command_template:
  ...
  type: template
command_topic:
  description: 用于发布命令以更改所选选项的 MQTT 主题。
  required: true
  type: string
default_entity_id:
  description: 使用 `default_entity_id` 代替名称来自动生成实体 ID。例如，`select.foobar`。当在没有 `unique_id` 的情况下使用时，如果实体 ID 可用，实体 ID 将在重新启动或重新加载期间更新。  如果实体 ID 已存在，则将创建实体 ID，并在末尾添加数字。与 `unique_id` 一起使用时，仅在首次添加实体时使用 `default_entity_id`。设置后，如果实体被删除并再次添加，这将覆盖用户自定义的实体 ID。
  required: false
  type: string
device:
  description: "有关此选择的设备的信息是将其绑定到 [device registry](https://developers.home-assistant.io/docs/en/device_registry_index.html) 的一部分。仅在设置 [`unique_id`](#unique_id) 时才有效。必须至少存在一个标识符或连接才能识别该设备。"
  required: false
  type: map
  keys:
    configuration_url:
...
json_attributes_topic:
  description: 订阅的 MQTT 主题接收 JSON 字典负载，然后设置为实体属性。当收到有关此主题的消息时，暗示当前选择状态的 `force_update`。
  required: false
  type: string
name:
  description: 选择的名称。如果仅设备名称相关，则可以设置为 `null`。
  required: false
  type: string
optimistic:
  description: 定义选择是否在乐观模式下工作的标志。
  required: false
  type: boolean
  default: "`true` if no `state_topic` defined, else `false`."
options:
  description: 可以选择的选项列表。允许使用空列表或包含单个项目的列表。
  required: true
  type: list
platform:
  description: 必须是 `select`。仅在 [MQTT auto discovery device messages](/home-assistant/integrations/mqtt/#device-discovery-payload) 中允许且要求。
  required: true
  type: string
qos:
...
retain:
  ...
  default: false
state_topic:
  description: 订阅 MQTT 主题以接收所选选项的更新。 “无”有效负载重置为 `unknown` 状态。空的有效负载将被忽略。
  required: false
  type: string
unique_id:
  description: 唯一标识此 Select 的 ID。如果两个 Select 具有相同的唯一 ID Home Assistant 将引发异常。与基于设备的发现一起使用时需要。
  required: false
  type: string
value_template:
