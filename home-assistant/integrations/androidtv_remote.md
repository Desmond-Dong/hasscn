...
此集成添加了一个具有基本播放和音量控制功能的 `media_player`。媒体播放器提供音量信息和 Android TV 上当前活动应用程序的显示名称。由于 API 限制，集成不会显示播放状态。建议将此集成与 [Google Cast 集成](/home-assistant/integrations/cast/index.md)一起使用。可以使用 [Universal Media Player](/home-assistant/integrations/universal/index.md) 集成将两个媒体播放器合并为一个。有关更多详细信息，请参阅[与 Google Cast 配合使用](#using-with-google-cast)部分。
