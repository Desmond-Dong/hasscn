...
这种整合。以下简约示例配置了几个
binary\_sensor，ID 为 0 的单个设备的开关和传感器端口。
...
0.0 和 1.0。使用可选的“source\_range”从特定范围映射并
...
*不是* Linux 设备路径。 Linux 设备路径（例如，`/dev/ttyACM0`）可以
...
以下步骤基于使用\_GNU Screen\_。在 Debian 或基于 Ubuntu 的操作系统上
像`sudo apt install screen`一样安装\_Screen\_。
...
3. 运行 `screen /dev/ttyACM0`
   4.输入“id get”查看当前ID
4. 输入“id set 00000005”并按 Enter 键分配 ID 5
5. 输入“id get”进行验证并期待“00000005”作为回复
6. 使用以下命令退出屏幕：Ctrl-a + \ 并使用 `y` 确认
