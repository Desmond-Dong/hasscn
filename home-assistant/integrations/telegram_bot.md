...
* 发送任何消息至 [@id\_bot](https://t.me/id_bot)。
...
要使 Telegram 机器人能够发送本地文件，您必须通过将文件的文件夹添加到 [`allowlist_external_dirs`](/home-assistant/integrations/homeassistant/index.md#allowlist_external_dirs) 来授予对其的访问权限。
...
2. 配置 [HTTP 集成](/home-assistant/integrations/http.md) 以允许 Home Assistant 接受来自反向代理的连接：
...
1. 转到 [**Settings** > **System** > **Network**](https://my.home-assistant.io/redirect/network/) 并配置 *Home Assistant URL*。
2. 通过配置以下变量来配置 [HTTP 集成](/home-assistant/integrations/http.md) 以在 Home Assistant 上启用 HTTPS：
...
发送内容 (`send_*`) 的操作将返回已传递消息的 `message_id`/`chat_id` 列表（在名为 `chats` 的属性中）。这将填充[响应数据](/home-assistant/docs/scripts/perform-actions.md#use-templates-to-handle-response-data)，您可以在自动化中进一步利用它，以便稍后根据 `message_id` 编辑/删除消息。有关使用说明，请参阅本页后面的示例。
...
| `chat_id` |是的 |用于发送通知的预先授权的 chat\_ids 或 user\_ids 数组。默认为第一个允许的 chat\_id。                                                                                                                                                                                       |
...
| `chat_id` |是的 |用于发送通知的预先授权的 chat\_ids 或 user\_ids 数组。默认为第一个允许的 `chat_id`。                                                                                                                                                                                     |
| `latitude` |没有|发送的纬度。                                                                                                                                                                                                                                                                                     |
| `longitude` |没有|发送的经度。                                                                                                                                                                                                                                                                                    |
| `disable_notification` |是的 | True/False 表示静默发送消息。 iOS 用户和网页用户不会收到通知，Android 用户会收到没有声音的通知。默认为 False。                                                                                                                            |
| `resize_keyboard` |是的 | True/False 用于调整键盘垂直大小以实现最佳贴合。默认为 False。                                                                                                                                                                                                                       |
...
| `chat_id` |是的 |在其中编辑reply\_markup的chat\_id。                                                                                                                                                                                                                                                               |
| `message_id` |没有|要编辑的消息的 ID。当从按下的按钮应答回调时，原始消息的 ID 位于：`{{ trigger.event.data.message.message_id }}`。您可以使用 `"last"` 来引用发送到 `chat_id` 的最后一条消息。                                                  |
| `inline_keyboard` |是的 |命令行列表，以逗号分隔，用于制作带有关联回调数据或外部 URL（仅限 https）的按钮的自定义内联键盘。示例：`["/button1, /button2", "/button3"]` 或 `[[["Text btn1", "/button1"], ["Text btn2", "/button2"]], [["Google link", "https://google.com"]]]` |
...
* 有关文件大小限制和下载行为，请参阅 python-telegram-bot 文档：[python-telegram-bot - get\_file](https://docs.python-telegram-bot.org/en/stable/telegram.bot.html#telegram.Bot.get_file)
...
|数据属性|可选|类型 |描述 |
| ---------------| -------- | ------- | --------------------------------------- |
| `chat_id` |没有|整数 |发送消息的目标 chat\_id。 |
...
请改用 [通知器](/home-assistant/integrations.md#notifiers)。

:::
[`telegram` 通知平台](/home-assistant/integrations/telegram.md) 需要 `telegram_bot` 集成才能使用，它旨在生成自定义快捷方式（`notify.USERNAME`），以使用旧语法将通知（消息、照片、文档和位置）发送到特定的 `chat_id`，从而实现向后兼容。还支持数据属性 `parse_mode`、`disable_notification`、`message_tag`、`disable_web_page_preview` 和 `message_thread_id`。
