...
无论如何，在报告问题时，请先启用[调试日志](/home-assistant/docs/configuration/troubleshooting/index.md#debug-logs-and-diagnostics)，重启集成，并在问题再次出现后立即关闭调试日志（*调试日志文件会自动开始下载*）。如果仍然可行，也请一并下载[诊断数据](/home-assistant/integrations/diagnostics.md)。收集到调试日志和诊断数据后，请随问题报告一同提供。
