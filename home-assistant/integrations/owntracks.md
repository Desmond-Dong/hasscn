...
默认情况下，集成通过 HTTP 监听来自 OwnTracks 的传入消息。如果 Home Assistant 配置为使用 MQTT，它将监听 MQTT 消息。当通过 HTTP 提交位置时，Home Assistant 会返回所有 [人员](/home-assistant/integrations/person/index.md) 的最后已知位置。他们的位置将显示在 OwnTracks 应用中。
