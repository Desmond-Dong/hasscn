...
对于其他逆变器系列，如果逆变器本身不能直接提供这些传感器，也可以通过现有传感器计算得到。可以使用 [Template Sensor](/home-assistant/integrations/template/index.md) 来拆分购电和售电功率值，再使用 [Riemann Sum](/home-assistant/integrations/integration/index.md) 将这些瞬时功率值（W）转换为累计能量值（Wh），然后用于能源仪表板。
