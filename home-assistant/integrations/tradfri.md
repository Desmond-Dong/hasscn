...
The Dirigera hub can, however, be integrated directly via the [HomeKit device](/home-assistant/integrations/homekit_controller/index.md) integration or the [Matter](/home-assistant/integrations/matter/index.md#using-a-matter-bridge) integration. As of Hub version 2.615.8 (September 2024), there is support for the following device types via Matter Bridge: lights (including drivers), smart plugs/outlets, connected blinds, remotes, motion sensors, open/close sensors, air purifiers, and air quality sensors.
...
* The TRÅDFRI Shortcut button, Remotes and motion sensor only send information about their battery status, no events, to Home Assistant and thus can't be used to automate with. If you want to automate with these devices, you need to use something like [ZHA](/home-assistant/integrations/zha/index.md), or the [HomeKit device](/home-assistant/integrations/homekit_controller.md) integration as mentioned above.
* The groups you find in the app are not imported into Home Assistant as they are known to cause stability issues. We recommend that you use the native [light groups](/home-assistant/integrations/group.md#binary-sensor-light-and-switch-groups) instead.
