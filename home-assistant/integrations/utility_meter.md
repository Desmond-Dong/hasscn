...
| ---------------------- | -------- | ----------- |
| `entity_id` | no | 指向 utility\_meter 的 `entity_id` 字符串或字符串列表。
...
以下配置示例中，2 个 utility\_meter（`daily_energy` 和 `monthly_energy`）分别跟踪每日和每月能耗。
...
当你使用 [DSMR 集成](/home-assistant/integrations/dsmr.md) 获取公用事业计量数据时，每个费率（高峰和低谷）都有独立传感器，燃气消耗也有独立传感器。计量器会自动切换费率，因此通常不需要自动化。但在这种情况下，你需要额外配置多个 `utility_meter` 实例。
...
我们使用 `is_number()` [函数](/home-assistant/docs/configuration/templating/index.md#numeric-functions-and-filters)
