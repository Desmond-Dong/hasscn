...
这些自动化可以通过 UI 配置（自动化中的相关说明请参阅 [device triggers](/home-assistant/docs/automation/trigger/index.md#device-triggers)）。如果您更喜欢 YAML，则需要从 UI 自动化编辑器中获取设备 ID。自动化格式如下：
...
使用此配置时，对 Kodi 设备调用 `media_player/turn_on` 会向指定 MAC 地址发送一个 *magic packet*。要使用此操作，您需要先在 Home Assistant 中配置 [`wake_on_lan`](/home-assistant/integrations/wake_on_lan.md) 集成，也就是在 `configuration.yaml` 中添加 `wake_on_lan:`。
...
可用的键盘按键名称请参阅：https://kodi.wiki/view/List\_of\_keynames
