...
无论如何，在报告问题时，请启用[调试日志](/home-assistant/docs/configuration/troubleshooting/index.md#debug-logs-and-diagnostics)，重启该集成，并在问题再次出现后立即停止调试日志（*调试日志文件会自动开始下载*）。此外，如果条件允许，也请下载[诊断](/home-assistant/integrations/diagnostics.md)数据。如果你已经收集了调试日志和诊断数据，请一并附在问题报告中。
