...
当通过 `script.turn_on` 动作调用一个或多个脚本时，调用方脚本\_不会\_等待。它会按列出的顺序依次启动脚本，并在最后一个脚本启动后立即继续执行。
被调用脚本中的错误导致其中止时，\_不会\_影响调用方脚本。
...
这种方式还可用于让调用方脚本等待被调用脚本结束，但在被调用脚本因错误中止时，调用方脚本\_不会\_被中止。
