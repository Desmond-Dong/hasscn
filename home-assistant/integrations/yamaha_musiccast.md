...
The Yamaha MusicCast integration implements the grouping actions. There are some limitations in the MusicCast system for grouping:
...
It is also possible to recall NetUSB presets using the play media action. To do so "presets:\<preset\_num>" has to be used as `media_content_id` in the action.
