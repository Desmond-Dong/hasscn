...
`media_player.select_source` 动作允许您在设备上启动应用。只需将应用 ID 作为 `source` 提供。您还可以通过在应用 ID 前加 `!` 来停止应用。例如，您可以定义[脚本](/home-assistant/docs/scripts.md)来启动和停止 Netflix，如下所示：
...
在您的[自动化设置](/home-assistant/getting-started/automation/index.md)的[动作](/home-assistant/getting-started/automation-action/)中，它可能如下所示：
...
例如，[脚本](/home-assistant/docs/scripts.md)中的动作可以从这样：
...
要确定这些规则应该是什么，您可以使用 `androidtv.adb_command` 动作和命令 `GET_PROPERTIES`，如 [androidtv.adb\_command](#androidtvadb_command) 部分所述。
