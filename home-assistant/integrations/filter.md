...
`filter` 将信号处理算法应用于传感器、先前和当前状态，并根据所选算法生成 `new state`。下图描绘了原始传感器和使用 [History Graph](/home-assistant/dashboards/history-graph/index.md) 集成的同一传感器的滤光片传感器。
...
:::warning
当配置不是时间且值大于默认值 `1` 的 `window_size` 时，数据库必须在 Home Assistant 启动期间检查该实体的几乎每个存储状态。如果您修改了 [Recorder `purge_keep_days`](/home-assistant/integrations/recorder/index.md#purge_keep_days) 值或在数据库中存储了筛选实体的许多状态，这可能会导致您的 Home Assistant 实例在启动期间响应不佳。
...
要调整速率，您需要设置 window\_size。要将传感器节流至 10%，`window_size` 应设置为 10，50% 应设置为 2。
...
要调整速率，您需要设置 window\_size。要将传感器节流至每分钟 1 个值，`window_size` 应设置为“00:01”。
