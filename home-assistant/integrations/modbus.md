...
此配置将每 15 秒轮询一次线圈地址 100 到 110，并更新 binary\_sensors：`my_relay`
和 `my_relay_1` 至 `my_relay_10`。

主设备配置（如 device\_class）会自动复制到从设备。
...
| count           | 是    | 是    | 否  | 否  | 否  |
| structure       | 是    | 否    | 否  | 否  | 否  |
| slave\_count     | 否    | 否    | 是  | 是  | 是  |
| virtual\_count   | 否    | 否    | 是  | 是  | 是  |
| swap: byte      | 否    | 否    | 是  | 是  | 是  |
| swap: word      | 否    | 否    | 否  | 是  | 是  |
| swap: word\_byte | 否    | 否    | 否  | 是  | 是  |
...
| 动作                 | 描述                 |
| --------------------- | --------------------------- |
| modbus.write\_register | 写入一个或多个寄存器 |
| modbus.write\_coil     | 写入一个或多个线圈         |
