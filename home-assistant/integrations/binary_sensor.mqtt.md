...
无状态设备（如按钮、遥控器等）更适合用 [MQTT 设备触发器](/home-assistant/integrations/device_trigger.mqtt/index.md) 而不是二值传感器来表示。
...
要在您的安装中使用 MQTT 二值传感器，请[将 MQTT 设备添加为子条目](/home-assistant/integrations/mqtt/index.md#configuration)，或将以下内容添加到您的 "`configuration.yaml`" 文件中。
:::tip
...
### 每次在 state\_topic 上收到消息时切换二值传感器
