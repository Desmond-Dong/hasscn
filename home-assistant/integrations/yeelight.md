...
自定义效果只能通过 YAML 配置。要开启效果，可使用 [light.turn\_on](/home-assistant/integrations/light/index.md#action-lightturn_on) 动作。
...
| `mode`                    |       no | 运行模式。可选值：'last'、'normal'、'rgb'、'hsv'、'color\_flow'、'moonlight'。                 |
...
`yeelight.set_color_flow_scene` 动作用于启动颜色流。与 [yeelight.start\_flow](#action-yeelightstart_flow) 的区别是它调用了不同的 Yeelight API。若灯原本关闭，将自动打开。不同固件在处理复杂流时可能存在差异。
...
`yeelight.set_music_mode` 动作用于启用或禁用 music\_mode。

| Data attribute    | Optional | Description                                                                                 |
|---------------------------|----------|---------------------------------------------------------------------------------------------|
| `entity_id`               |      yes | Only act on specific lights.                                                                |
| `music_mode`              |       no | Use 'true' or 'false' to enable / disable music\_mode.                                       |
