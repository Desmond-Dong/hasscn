...
*提示：* 使用动作后点击侧边栏的通知，按照说明确保您在正确的时间按下按钮。
...
已学习的代码存储在 `/config/.storage/` 中的 `broadlink_remote_MACADDRESS_codes` JSON 文件中。您可以使用文本编辑器打开此文件并复制代码来设置[自定义 IR/RF 开关](#setting-up-custom-irrf-switches)或将其作为 [base64 代码](#sending-a-base64-code)发送，但请注意：.storage 文件夹中的文件\_不应手动编辑\_。
...
**重要**：始终为您的开关使用唯一名称。一个好的选择是在名称前加上设备所在区域的名称，例如 Bedroom TV。
