...
持久通知[触发器](/home-assistant/docs/automation/trigger.md)可在持久通知更新时触发自动化。可通过为 `notification_id` 提供 ID 将触发器限制为特定通知；若省略该值，则自动化会对任意通知 ID 触发。若未提供 `update_type`，自动化会在以下更新类型时触发：`added`、`removed`、`updated` 或 `current`。若在 `update_type` 选项中提供其中一个或多个值，则仅在这些 `update_type` 事件发生时触发。

完整细节请参阅 [Home Assistant 自动化入门](/home-assistant/getting-started/automation/index.md)指南或[自动化](/home-assistant/docs/automation/index.md)文档。
...
可用于条件或动作的更多触发数据，请参阅[自动化触发器变量：Persistent Notification](/home-assistant/docs/automation/templating/index.md#persistent-notification)。
...
下面是你的[自动化配置](/home-assistant/getting-started/automation/index.md)中使用静态内容的[action](/home-assistant/docs/automation/action.md)示例。
...
如果你想显示运行时信息，需要使用[模板](/home-assistant/docs/configuration/templating/index.md)。
...
如果已加载 [Notify 集成](/home-assistant/integrations/notify/index.md)，持久通知也可作为预配置通知器使用，对应实体为 `notify.persistent_notification`。这样即可用于需要通知器的功能，例如 [通知组](/home-assistant/integrations/group/index.md#notify-groups) 或 [Alert 集成](/home-assistant/integrations/alert/index.md)。
