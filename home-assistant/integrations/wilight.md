...
| `set_trigger` | Calling this action sets the trigger\_1, trigger\_2, trigger\_3 or trigger\_4 (`entity_id`, `trigger_index` and `trigger` are required parameters), `trigger_index` must be between 1 and 4 and `trigger` must be according Trigger rules (see below).
