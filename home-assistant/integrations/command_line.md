...
有关如何使用通知，请参阅[自动化入门](/home-assistant/getting-started/automation/index.md)。
...
`command` 会在[配置目录](/home-assistant/docs/configuration/index.md)中执行。
...
本节提供了一些如何使用 command\_line 传感器的实际示例。
...
也可以考虑使用 [ping sensor](/home-assistant/integrations/ping.md#binary-sensor) 作为上述示例的替代方案。
...
此示例与 [aREST sensor](/home-assistant/integrations/arest.md#sensor) 实现的功能相同，但使用的是外部 Python 脚本。它可以帮助您了解如何与公开 RESTful API 的设备进行交互。
...
`command` 配置变量支持使用[模板](/home-assistant/docs/configuration/templating/index.md)。如果您想将某个特定传感器的状态作为参数传递给外部脚本，就可以使用此功能。
...
下面的示例与 [aREST switch](/home-assistant/integrations/arest.md#switch) 实现的功能相同。
...
基于此示例，在 UI 中您会看到 `friendly_name` 为 “Kitchen Light”。不过，其 `identifier` 是 `arest_pin_four`，因此 `entity_id` 会是 `switch.arest_pin_four`，这也是您在[`automation`](/home-assistant/integrations/automation/index.md)或 [API 调用](/home-assistant/developers/index.md)中会使用的值。
