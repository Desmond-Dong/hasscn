[HomeKit](https://developer.apple.com/apple-home/) Device 集成可让你将带有“Works with HomeKit”标识的配件连接到 Home Assistant。请不要将此集成与 [HomeKit Bridge](/home-assistant/integrations/homekit/index.md) 集成混淆，后者可让你通过 HomeKit 控制 Home Assistant 设备。
...
* 如果你没有启用 [`default_config`](/home-assistant/integrations/default_config/index.md) 集成，请在 "`configuration.yaml`" 文件中添加 [`zeroconf`](/home-assistant/integrations/zeroconf/index.md)。
...
4. 当 Home Assistant 完成设备配置后，你可以通过 [`HomeKit Bridge`](/home-assistant/integrations/homekit/index.md) 集成将它重新导出到 Siri 和 Apple Home。

## 通过蓝牙添加 HomeKit 设备

你可以通过 [Bluetooth](/home-assistant/integrations/bluetooth.md) 将 HomeKit [兼容设备](#supported-devices)添加到 Home Assistant。
...
某些 HomeKit 设备（如按钮、遥控器和门铃）不像普通 HomeKit 设备那样提供可读取的传感器，它们只会在事件发生时通知 Home Assistant。这意味着 Home Assistant 无法为它们显示实体，因为它们没有状态。不过，你仍可以将它们用于[设备自动化](/home-assistant/integrations/device_automation/index.md)。
