...
5. 选择您首选的 AWS **region**（例如 `eu-central-1`）。
6. 调整设置：
...
2. 点击 **Add users**。
3. 使用类似 `home-assistant-backup` 的名称。
4. 仅勾选 **Access key - Programmatic access**。
...
2. 为策略命名（例如 `HomeAssistantS3Policy`）并创建它。

3. 返回用户创建向导并附加新策略。

4. 完成用户设置。

5. 保存 **Access Key ID** 和 **Secret Access Key** — 在 Home Assistant 中设置 AWS S3 集成时您需要这些。
...
2. 点击 **Add Integration** 并搜索 **AWS S3**。
3. 输入以下详细信息：
