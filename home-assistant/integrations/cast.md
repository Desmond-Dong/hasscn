...
:::important
Home Assistant Cast 要求你的 Home Assistant 实例可通过 `https://` 访问。如果你使用 Home Assistant Cloud，则无需额外操作。否则你必须在 [configuration](/home-assistant/integrations/homeassistant/index.md#configuration-variables) 中配置 `external_url`。
...
这在投放 TTS 或本地媒体源时尤为重要；cast 集成会使用本地 Home Assistant URL 来投放这些媒体。你可以在 **[Settings > System > Network](https://my.home-assistant.io/redirect/network/)** 中设置，或通过配置 [`internal_url`](/home-assistant/integrations/homeassistant/index.md#editing-the-general-settings-in-yaml) 来设置。
...
|          |                                 |
| -------- | ------------------------------- |
| 2908kbps | dash (mf\_cloudfront\_dash\_https) |
| b09w70r2 | 960x540                         |
...
若要直接从已配置的 Plex 服务器投放媒体，请按 [as documented in the Plex integration](/home-assistant/integrations/plex/index.md#action-media_playerplay_media) 设置相关字段，并在 `media_content_id` 前加上 `plex://`：
...
|          |                                         |
| -------- | --------------------------------------- |
| 128bps   | dash (mf\_cloudfront\_nonbidi\_dash\_https) |
| p009ycqz |                                         |
...
Google Cast 集成依赖 [Zeroconf integration](/home-assistant/integrations/zeroconf.md) 实现 mDNS 发现。Zeroconf 集成中的部分配置项会影响 mDNS 路由。
...
当你在 Docker 中运行 [Home Assistant Container](/home-assistant/installation/linux.md#install-home-assistant-container) 时，请确保使用 host 网络模式。Home Assistant 项目不支持非 host 网络模式，这会导致该集成无法发现你的 Cast 设备。
