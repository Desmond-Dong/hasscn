...
The **MQTT Switch** integration lets you control your MQTT enabled switches.

## Configuration

在理想情况下，MQTT 设备将有一个“state\_topic”来发布状态更改。如果这些消息使用“RETAIN”标志发布，则 MQTT 交换机将在订阅后收到即时状态更新，并以正确的状态开始。否则，开关的初始状态将为“未知”。 MQTT 设备可以使用“None”有效负载将当前状态重置为“unknown”。

当“state\_topic”不可用时，交换机将以乐观模式工作。在此模式下，开关将在每个命令后立即更改状态。否则，交换机将等待来自设备的状态确认（来自“state\_topic”的消息）。在乐观模式下，初始状态设置为“False”/“off”。

即使“state\_topic”可用，也可以强制使用乐观模式。如果遇到不正确的开关操作，请尝试启用它。

To use an MQTT switch in your installation, [add a MQTT device as a subentry](/home-assistant/integrations/mqtt/index.md#configuration), or add the following to your `configuration.yaml` file.
After changing the `configuration.yaml` file, [restart Home Assistant](/home-assistant/docs/configuration/index.md#reloading-the-configuration-to-apply-changes) to apply the changes.
...
availability_topic:
  ...
  type: string
command_template:
  description: Defines a [template](/home-assistant/docs/configuration/templating/#using-command-templates-with-mqtt) to generate the payload to send to `command_topic`. The switch command template accepts the parameters `value`. The `value` parameter will contain the configured value for either `payload_on` or `payload_off`.
  required: false
  type: template
command_topic:
...
device:
  ...
  keys:
    ...
    via_device:
      ...
      type: string
device_class:
  description: The [type/class](/home-assistant/integrations/switch/#device-class) of the switch to set the icon in the frontend. The `device_class` can be `null`.
  required: false
  type: device_class
enabled_by_default:
...
payload_on:
  ...
  default: "ON"
platform:
  description: Must be `switch`. Only allowed and required in [MQTT auto discovery device messages](/home-assistant/integrations/mqtt/#device-discovery-payload).
  required: true
  type: string
qos:
...
unique_id:
  ...
  type: string
value_template:
  description: "Defines a [template](/home-assistant/docs/configuration/templating/#using-value-templates-with-mqtt) to extract device's state from the `state_topic`. To determine the switches's state result of this template will be compared to `state_on` and `state_off`."
  required: false
  type: template
```
...
为了进行检查，您可以使用“mosquitto”附带的命令行工具“mosquitto\_pub”来发送 MQTT 消息。这允许您手动操作开关。首先，我们可以模拟为交换机发送的可用性消息：

```bash
mosquitto_pub -h 127.0.0.1 -t home/bedroom/switch1/available -m "online"
```

我们可以通过发布“ON”命令消息来模拟开关被打开：

```bash
mosquitto_pub -h 127.0.0.1 -t home/bedroom/switch1/set -m "ON"
```

最后，我们可以模拟开关将更改后的状态报告给 Home Assistant：
...
假设您已经使用 [ESPEasy](https://github.com/letscontrolit/ESPEasy) 刷新了 ESP8266 单元。 “配置”下是为您的设备设置的名称（“单位名称：”）（此处为“浴室”）。存在协议“OpenHAB MQTT”的 MQTT“控制器”配置，并且条目（“控制器订阅：”和“控制器发布：”）会根据您的需求进行调整。在此示例中，主题以“home”为前缀。无需进一步配置，因为可以直接使用 MQTT 控制 [GPIO](https://espeasy.readthedocs.io/en/latest/Controller/C005.html)。

您可以使用“mosquitto\_pub”或其他 MQTT 工具手动将引脚 13 设置为高电平：
