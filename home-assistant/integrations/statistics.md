...
如果 [`recorder`](/home-assistant/integrations/recorder/index.md) 集成正在运行，平台启动时会从数据库中读取历史传感器数据，因此在重启后可立即获得数据。如果 [`recorder`](/home-assistant/integrations/recorder/index.md) 集成*未*运行，传感器开始输出数据可能需要一些时间，因为某些统计特征的计算需要不止一个源传感器值。
