...
其中 **namespace** 是当前正在记录日志的 *\<component\_namespace>*。
...
对于 Home Assistant Container 安装，日志信息会存储在[配置目录](/home-assistant/docs/configuration/index.md)中的 `home-assistant.log` 文件里。
您可以使用命令行工具 `cat` 读取，或使用 `tail -f` 动态查看。

当您通过 [Home Assistant 的 SSH 应用](/home-assistant/common-tasks/os/index.md#installing-and-using-the-ssh-app)（以前称为 SSH add-on）登录后，可以使用以下示例：
