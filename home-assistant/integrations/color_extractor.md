...
由于 `color_extractor.turn_on` 会调用 `light.turn_on`，您可以传递任何有效的 [`light.turn_on`](/home-assistant/integrations/light.md#action-lightturn_on) 参数（`rgb_color` 会为您设置）。
...
:::important
确保任何 [外部 URL](/home-assistant/integrations/homeassistant/index.md#allowlist_external_urls) 或 [外部文件](/home-assistant/integrations/homeassistant/index.md#allowlist_external_dirs) 已授权使用。如果不允许此集成访问这些外部资源，您将收到错误消息。
...
带有 5 秒的过渡效果，每次将亮度设置为 100%（[`light.turn_on`](/home-assistant/integrations/light.md#action-lightturn_on) 动作参数的一部分）：
