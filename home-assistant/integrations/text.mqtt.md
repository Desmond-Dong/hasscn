...
To use an MQTT text entity in your installation, [add an MQTT device as a subentry](/home-assistant/integrations/mqtt/index.md#configuration), or add the following to your `configuration.yaml` file.
...
pattern:
  ...
  type: string
platform:
  description: Must be `text`. Only allowed and required in [MQTT auto discovery device messages](/home-assistant/integrations/mqtt/#device-discovery-payload).
  required: true
  type: string
qos:
...
unique_id:
  ...
  type: string
value_template:
  description: "Defines a [template](/home-assistant/docs/configuration/templating/#using-value-templates-with-mqtt) to extract the text state value from the payload received on `state_topic`."
  required: false
  type: template
```
