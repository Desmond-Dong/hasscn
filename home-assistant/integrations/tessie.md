...
能源历史传感器提供累计能量值（kWh），可直接用于[能源仪表板](/home-assistant/docs/energy/index.md)。

另外，你也可以基于 `Battery power` 和 `Grid power` 传感器，通过 [Template Sensor](/home-assistant/integrations/template/index.md) 将正负值拆分为导入和导出值，以此计算能量流向。
然后，`Load power`、`Solar power` 以及这些模板传感器可以使用 [Riemann Sum](/home-assistant/integrations/integration/index.md) 将瞬时功率值（kW）转换为累计能量值（kWh），
从而用于能源仪表板。
...
Tessie 集成支持[诊断数据收集](/home-assistant/docs/configuration/troubleshooting/index.md#download-diagnostics)，可帮助排查问题。如果你在使用集成时遇到问题，可以下载诊断信息并在报告问题时一并提交。
