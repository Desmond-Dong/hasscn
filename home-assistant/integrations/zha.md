...
**Zigbee 家庭自动化** (ZHA) integration 允许您使用称为\_Zigbee 协调器\_ 的众多兼容硬件适配器之一将许多现成的 [Zigbee-based devices](https://csa-iot.org/csa-iot_products/) 直接无线连接到 Home Assistant。
...
* [Button](/home-assistant/integrations/button/index.md)
* [Climate](/home-assistant/integrations/climate/index.md)（测试版）
* [Cover](/home-assistant/integrations/cover/index.md)
* [Device tracker](/home-assistant/integrations/device_tracker/index.md)
* [Fan](/home-assistant/integrations/fan/index.md)
* [Light](/home-assistant/integrations/light/index.md)
* [Lock](/home-assistant/integrations/lock/index.md)
* [Number](/home-assistant/integrations/number/index.md)（即模拟输入/输出）
* [Select](/home-assistant/integrations/select/index.md)
* [Sensor](/home-assistant/integrations/sensor/index.md)
* [Siren](/home-assistant/integrations/siren/index.md)
* [Switch](/home-assistant/integrations/switch/index.md)
* [Update](/home-assistant/integrations/update/index.md)
...
* **Zigbee组**：两个或多个相同类型的Zigbee设备的集合，与Home Assistant的[Groups](/home-assistant/integrations/group/index.md)不同。
...
支持以下硬件，但\_不推荐\_。每个部分均注明了具体型号和详细信息。
...
`{ieee}` 是设备硬件地址，在查看\_设备信息\_时可以从 Home Assistant UI 读取。从设备信息中，您可以通过查看\_Zigbee设备签名\_找到`{endpoint_id}`。

### Zigbee 设备 OTA 固件更新 {#zigbee-device-firmware-ota-updates}

ZHA 集成能够执行 Zigbee 设备的 OTA（无线）固件更新。该功能默认启用。由于它在 Home Assistant 中使用标准 [Update](/home-assistant/integrations/update/index.md) 实体，因此如果特定设备有可用的 OTA 固件更新，用户将收到 UI 通知，并可以选择启动更新或忽略该设备的特定更新。
...
2. 根据制造商提供的设备说明将 Zigbee 设备重置为出厂默认设置（例如，打开/关闭灯最多 10 次；开关通常具有重置按钮/引脚）。设备可能需要几秒钟的时间才会出现。您可以单击 **显示日志** 以获取更详细的输出。
3. 找到设备后，它将出现在该页面上，并自动添加到您的设备中。您可以选择更改其名称并将其添加到区域（您可以稍后更改）。您可以再次搜索以添加其他设备，也可以返回到已添加的设备列表。
...
ZHA 支持创建 Zigbee 组（与 Home Assistant 的 [Group](/home-assistant/integrations/group/index.md) 集成不同），以及设备之间的绑定。组和设备绑定可以彼此独立设置，但也可以组合使用（例如将一个设备绑定到另一组设备）。
...
:::note
虽然使用本机 Zigbee 组而不是 Home Assistant 的 [Group](/home-assistant/integrations/group/index.md) 集成可以提高视觉响应能力，但如果重复发出，发出的广播命令可能会淹没 Zigbee 网络。
...
* 无线电类型是以下之一：
  ...
  * znp *（德州仪器 Z-Stack ZNP）*
  * deCONZ *（德累斯顿电子公司的 ConBee/RaspBee）*
...
此非官方列表包含独立社区成员针对多种家庭自动化网关/网桥/集线器软件（包括 ZHA、Zigbee2MQTT 和 Tasmota/Zigbee2Tasmota 等开源 Zigbee 实现）的报告\_（或特定于设备的配对技巧）\_。
...
3. 还没工作？您可能需要一个自定义设备处理程序。该处理程序将具有异常处理代码来解决特定于设备的问题。
...
要启用 ZHA integration 和无线电库的调试日志记录，请将以下 [logger](/home-assistant/integrations/logger/index.md) 配置添加到 B64PODc=P64B：
...
* **Zigbee 遥控器：**
  * 下面概述了 *Philips Hue 调光器开关* 或 *Lutron Connected Bulb Remote* 的步骤。
  * 遥控器不必与您之前的桥配对。
* **通过 Android 应用程序蓝牙：**
