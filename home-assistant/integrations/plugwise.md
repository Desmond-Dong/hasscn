...
2. **通过 Home Assistant 控制**：
   * 使用气候卡片激活/停用日程。
   * 'Auto' 模式表示日程处于活动状态。
   * 'Heat'、'Cool' 或 'Heat\_cool' 模式表示日程处于非活动状态。
...
有关如何使用可用动作的信息，请参阅[气候](/home-assistant/integrations/climate.md#climate-control-actions)集成。
...
下面的示例自动化将更改您的 Adam 的网关模式为假日模式（和返回），假设您有一个[日历](/home-assistant/integrations/calendar.md)集成，其中设置了无人在家时的特定日历事件。
