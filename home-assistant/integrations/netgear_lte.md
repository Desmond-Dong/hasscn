...
到达调制解调器收件箱的消息将作为“netgear\_lte\_sms”类型的事件发送，并包含以下内容。
...
“netgear\_lte.connect\_lte”操作要求调制解调器建立其 LTE 连接，如果调制解调器不自动连接，则该操作很有用。
...
“netgear\_lte.disconnect\_lte”操作要求调制解调器关闭其 LTE 连接。
...
“netgear\_lte.delete\_sms”操作会从调制解调器收件箱中删除消息。这可用于在传入 SMS 事件后进行清理。
...
以下自动化示例使用 [Conversation](/home-assistant/integrations/conversation/index.md) 集成处理传入的 SMS 消息，然后从收件箱中删除该消息。
