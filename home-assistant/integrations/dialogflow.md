...
为了能够从 Dialogflow 接收消息，您的 Home Assistant 实例需要可以从网络访问，并且您需要[配置](/home-assistant/integrations/homeassistant/index.md#editing-the-general-settings-in-yaml)外部 URL。如果您的服务器没有响应或响应时间过长（超过 5 秒），Dialogflow 将返回后备答案。
...
激活后，[`alexa` 集成](/home-assistant/integrations/alexa/index.md) 将让 Home Assistant 的本机意图支持处理传入的意图。如果您想基于意图运行动作，请使用 [`intent_script`](/home-assistant/integrations/intent_script.md) 集成。
