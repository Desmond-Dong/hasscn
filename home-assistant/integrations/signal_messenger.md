...
* 要附加 Home Assistant 外部的文件，这些 URL 必须可访问，并加入 [`allowlist_external_urls`](/home-assistant/integrations/homeassistant/index.md#allowlist_external_urls) 列表。
...
为实现这一点，请确保插件的 `mode` 参数设置为 `native` 或 `normal`，然后编辑 Home Assistant 配置，按如下方式添加一个 [RESTful resource](/home-assistant/integrations/rest/index.md)：
