...
**Google Photos** 集成允许您使用 Home Assistant 将照片上传到您的 [Google Photos](https://photos.google.com/)。该集成添加了用于上传照片的动作和[媒体源](/home-assistant/integrations/media_source.md)来查看或投射由 Home Assistant 上传的内容。此集成不会让 Home Assistant 访问您的整个 Google Photos 媒体库。

## 前提条件

您需要配置开发者凭据以允许 Home Assistant 访问您的 Google 账户。
这些凭据与 [Nest](/home-assistant/integrations/nest.md)、[Google Tasks](/home-assistant/integrations/google_tasks.md) 和 [Google Mail](/home-assistant/integrations/google_mail.md) 的凭据相同。
这些与之前为 [Google Calendar](/home-assistant/integrations/google.md) 推荐的 *设备授权* 凭据不同。
...
集成设置接下来会为您提供说明，以输入[应用凭据](/home-assistant/integrations/application_credentials/index.md)（OAuth 客户端 ID 和客户端密钥）并授权 Home Assistant 访问您的 Google Photos。
...
4. 页面现在将显示 **将账户链接到 Home Assistant？**，注意 **您的实例 URL**。如果不正确，请参阅 [My Home Assistant](/home-assistant/integrations/my.md)。如果一切看起来正确，选择 **链接账户**。
...
如果您的凭据有错误，您可以在 [应用凭据](/home-assistant/integrations/application_credentials/index.md) 用户界面中删除它们。
