...
* 为每个支持 HEOS 的产品提供 [媒体播放器](/home-assistant/integrations/media_player.md) 实体，包括扬声器、功放和接收器（Denon 和 Marantz）。
* 查看当前正在播放的媒体。
...
1. 一个或多个[支持 HEOS 的设备](/home-assistant/integrations/heos.md#supported-devices)。
...
3. Enter a new [host name or IP address](/home-assistant/integrations/heos/index.md#host).
...
除了标准的[媒体播放器操作](/home-assistant/integrations/media_player.md#actions)外，HEOS 集成还提供以下操作：
...
:::note
如果 HEOS 设备无法处理某个操作，该操作可能会失败。例如，当队列为空时调用 `media_player.clear_playlist` 会导致错误。要避免这类错误中断脚本或自动化，可在操作调用中设置 [`continue_on_error: true`](/home-assistant/docs/scripts/index.md#continuing-on-error)。
...
* AVR receiver features, such as zone selection/control and power on/off, cannot be controlled through this integration. Use the [Universal Media Player](/home-assistant/integrations/universal/index.md#denon-avr--heos) to combine AVR receiver functionality with this integration.
...
HEOS 集成支持 [Home Assistant 调试日志与诊断](/home-assistant/docs/configuration/troubleshooting/index.md#debug-logs-and-diagnostics)。诊断信息可在集成条目级别和设备级别获取。
...
集成启动时会记录上述消息，并且集成的 media\_player 实体中的 `source_list` 属性会为空。此时尝试调用 `media_player.play_media` 来播放 `favorite` 或 `playlist` 会失败。集成的其他功能不受影响。
