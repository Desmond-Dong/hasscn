...
请按照[获取访问令牌](/home-assistant/integrations/xiaomi_miio/index.md#retrieving-the-access-token)中的说明获取 API 令牌。
...
* 属性
  ...
  * battery
  * time\_stat
  * carbon\_dioxide\_equivalent
  * total\_volatile\_organic\_compounds
  * temperature
  * humidity
...
请按照[获取访问令牌](/home-assistant/integrations/xiaomi_miio/index.md#retrieving-the-access-token)章节中的说明，获取用于 "`configuration.yaml`" 的 API 令牌。
...
原始码命令是通过 [`xiaomi_miio.remote_learn_command`](/home-assistant/integrations/xiaomi_miio/index.md#xiaomi_miioremote_learn_command) 学习得到的命令。
...
允许通过命令标识符发送已命名命令，或按[命令类型](/home-assistant/integrations/xiaomi_miio/index.md#command-types)中定义的两种格式之一发送命令。

### `xiaomi_miio.remote_learn_command`

用于学习新命令。

使用 Xiaomi IR Remote 的 entity\_id 来启动学习流程。

可以指定 `slot` 和 `timeout`，即使同一槽位中的命令被覆盖，仍可通过 [`remote.send_command`](/home-assistant/integrations/xiaomi_miio/index.md#remotesend_command) 发送已学习的多个命令。
...
`xiaomi` 平台的吸尘器不会暴露除 [`vacuum` 集成](/home-assistant/integrations/vacuum/index.md#attributes) 已提供属性以外的额外属性。

### 示例：如何清扫指定房间

使用 [`vacuum.send_command`](/home-assistant/integrations/vacuum/index.md) 清扫指定房间的脚本示例：
...
吸尘器实体会保存刷子、滤网与传感器需要清洁或更换的属性值（`main_brush_left`、`side_brush_left`、`filter_left` 和 `sensor_dirty_left`），单位为小时。清洁或更换对应部件后，你可以在吸尘器上重置这些值。下面示例展示了如何使用 [`vacuum.send_command`](/home-assistant/integrations/vacuum/index.md) 重置主刷工时：
...
:::note
如果使用 Android 设备获取访问令牌，目前仅确认 Mi Home `v5.4.49` 可用（2019 年 12 月）。使用 `v5.4.49` 在 `Smarthome/logs` 文件夹下查找保存 32 位令牌的文本文件。该目录通常会有多个文本文件，请在所有文件中搜索 `token`。请注意，最新版本 Mi Home 不会以明文存储令牌。 <br/> <br/>
iPhone 应用在 `v4.23.4`（2019 年 11 月 17 日）时仍会将令牌存储在 SQLite 数据库中。 <br/> <br/>
重置小米扫地机器人 Wi-Fi 设置后会生成新的访问令牌，因此需要重新执行这些步骤。 <br/> <br/>
这些说明适用于 Mi Home 应用，不适用于新版 RoboRock 应用。 <br/> <br/>
...
6. 查找类似 `123456789_mihome.sqlite` 的文件（注意：`_mihome.sqlite` *不是* 正确文件。通常可在 `Documents` 文件夹中找到）。
