...
如果您使用的是 Home Assistant 操作系统，可以通过[安装](/home-assistant/common-tasks/os.md#installing-third-party-apps)来自 [pschmitt 的仓库](https://github.com/pschmitt/home-assistant-addons) 的 Home Assistant flicd 应用程序在本地运行该服务。
...
* **button\_name**：触发事件的按钮名称。
* **button\_address**：触发事件的按钮蓝牙地址。
* **click\_type**：点击类型。可能的值有 `single`、`double` 和 `hold`。
* **queued\_time**：此事件在按钮上排队的时间（秒）。

为了帮助检测和调试 flic 按钮点击，您可以使用此自动化，在每次点击任何按钮时发送通知。此示例使用 [HTML5 推送通知平台](/home-assistant/integrations/html5.md)。访问[通知集成页面](/home-assistant/integrations/notify/index.md)了解更多关于设置通知的信息。
