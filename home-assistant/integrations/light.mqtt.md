...
要在你的安装中使用 MQTT 基础灯光，请[将 MQTT 设备作为子条目添加](/home-assistant/integrations/mqtt/index.md#configuration)，或将以下内容添加到 `configuration.yaml` 文件中。
:::tip
...
此外，也可以通过更高级的 [MQTT 发现](/home-assistant/integrations/mqtt/index.md#mqtt-discovery) 方式进行设置。
...
要在你的安装中使用 MQTT JSON 灯光，请[将 MQTT 设备作为子条目添加](/home-assistant/integrations/mqtt/index.md#configuration)，或将以下内容添加到 `configuration.yaml` 文件中。
:::tip
更改配置后需要重启 Home Assistant。
:::
...
* [MQTT JSON Light](https://github.com/mertenats/Open-Home-Automation/tree/master/ha_mqtt_rgbw_light_with_discovery) 是另一个用于 ESP8266 的实现，并包含 [MQTT discovery](/home-assistant/integrations/mqtt/index.md#mqtt-discovery)。

* [ESPHome](https://esphome.io) 为基于 MQTT 的安装实现了 JSON 架构，并支持 [MQTT discovery](/home-assistant/integrations/mqtt/index.md#mqtt-discovery)。

* [AiLight](https://github.com/stelgenhof/AiLight) 是用于 Ai-Thinker（及同类）RGBW WiFi 灯泡的自定义固件，设备内置 ESP8266 并由 MY9291 LED 驱动控制。它实现了 [MQTT JSON light](/home-assistant/integrations/light.mqtt.md) 平台，支持开关、RGBW 颜色、亮度、色温、闪烁和过渡。同时还包含 [MQTT Auto Discovery](/home-assistant/integrations/mqtt/index.md#mqtt-discovery)，并启用了 MQTT 遗嘱消息（LWT）。

* [h801-mqtt-json](https://github.com/starkillerOG/h801-mqtt-json) 是面向 H801 LED 调光器的自定义固件。H801 是一款 5 通道（RGBWWCW）的 12V WiFi 灯带控制器。该固件用于控制 H801 的 5 个通道，可同时控制 RGB 灯带和暖白/冷白灯带，例如 5050 RGB 灯带与 5025 双白灯带。它实现了 [MQTT JSON light](/home-assistant/integrations/light.mqtt.md) 平台，支持开关、RGB 颜色（RGB 灯带）、亮度、色温（冷白/暖白灯带）和过渡。
...
要在你的安装中使用 MQTT 模板灯光，请[将 MQTT 设备作为子条目添加](/home-assistant/integrations/mqtt/index.md#configuration)，或将以下内容添加到 "`configuration.yaml`" 文件中。
:::tip
更改配置后需要重启 Home Assistant。
:::
...
此外，更高级的方式是通过 [MQTT discovery](/home-assistant/integrations/mqtt/index.md#mqtt-discovery) 进行设置。
